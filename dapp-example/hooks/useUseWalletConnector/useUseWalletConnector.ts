import type { IARC0001Transaction, BaseARC0027Error } from '@agoralabs-sh/avm-web-provider';
import { decode as decodeBase64, encode as encodeBase64 } from '@stablelib/base64';
import { WalletId, type WalletAccount, WalletManager } from '@txnlab/use-wallet';
...
// types
import type { INetwork, INode } from '@provider/types';
import type { IAccountInformation, IConnectorParams, IConnectorState } from '../../types';

// utils
import getRandomItem from '@common/utils/getRandomItem';
import { getAccountInformation } from '../../utils';

export default function useUseWalletConnector({ toast }: IConnectorParams): IConnectorState {
  // memos
  ...
  const connectAction = async (network: INetwork) => {
    ...
    const wallet = _walletManager.wallets.find((value) => value.id === WalletId.KIBISIS) || null;
    let walletAccounts: WalletAccount[];

    if (!wallet) {
      logger.error(`${__hook}#${__function}: failed to get kibisis wallet in use-wallet`);
    ...
    try {
      setEnabledAccounts(
        await Promise.all(
          walletAccounts.map<Promise<IAccountInformation>>(({ address, name }) =>
  ...
  const disconnectAction = async () => {
    ...
    const wallet = walletManager?.wallets.find((value) => value.id === WalletId.KIBISIS) || null;

    setEnabledAccounts([]);

    if (!wallet) {
      logger.error(`${__hook}#${__function}: failed to get wallet from use-wallet wallet manager`);
  ...
  const signTransactionsAction = async (transactions: IARC0001Transaction[]) => {
    ...
    result = await wallet.signTransactions(transactions.map(({ txn }) => decodeBase64(txn)));

    return result.map((value) => (value !== null ? encodeBase64(value) : value));
  };
