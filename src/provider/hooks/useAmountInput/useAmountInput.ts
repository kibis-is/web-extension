...
// types
import type { IOnEventOptions } from '@provider/components/assets/AmountInput';
...
export default function useAmountInput({ label, required }: IOptions): IState {
  ...
  const _onBlur =
    ...
    (event: FocusEvent<HTMLInputElement>) => {
      ...
      _value = new BigNumber(event.target.value.length > 0 ? event.target.value : '0');
      maximumAmountInStandardUnits = convertToStandardUnit(new BigNumber(maximumAmountInAtomicUnits), asset.decimals);
  ...
  const _onMaximumAmountClick = ({ asset, maximumAmountInAtomicUnits }: IOnEventOptions) => {
    if (!asset || !maximumAmountInAtomicUnits) {
      return;
    }

    setValue(convertToStandardUnit(new BigNumber(maximumAmountInAtomicUnits), asset.decimals).toString());
  };
  const reset = () => {
    setValue('0');
  };
