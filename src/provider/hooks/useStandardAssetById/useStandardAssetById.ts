...
export default function useStandardAssetById(id: string): IUseStandardAssetByIdState {
  // selectors
  ...
  // state
  const [standardAsset, setStandardAsset] = useState<IStandardAsset | null>(null);

  // fetch unknown asset information
  useEffect(() => {
    const selectedAsset: IStandardAsset | null = standardAssets.find((value) => value.id === id) || null;
