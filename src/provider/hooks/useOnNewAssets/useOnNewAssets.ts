...
// features
import { updateARC0072AssetInformationThunk } from '@provider/features/arc0072-assets';
  ...
  useEffect(() => {
    if (accounts.length > 0 && arc0072Assets && selectedNetwork) {
      accounts.forEach((account) => {
        const encodedGenesisHash = convertGenesisHashToHex(selectedNetwork.genesisHash).toUpperCase();
        const accountInformation = account.networkInformation[encodedGenesisHash] || null;
        ...
        if (accountInformation) {
          ...
          newARC0072AssetHoldings = accountInformation.arc0072AssetHoldings.filter(
            (assetHolding) => !arc0072Assets.some((value) => value.id === assetHolding.id)
          );
    ...
    if (accounts.length > 0 && arc0200Assets && selectedNetwork) {
      accounts.forEach((account) => {
        ...
        if (accountInformation) {
          ...
          newARC0200AssetHoldings = accountInformation.arc200AssetHoldings.filter(
            (assetHolding) => !arc0200Assets.some((value) => value.id === assetHolding.id)
          );
    ...
    if (accounts.length > 0 && standardAssets && selectedNetwork) {
      accounts.forEach((account) => {
        ...
        if (accountInformation) {
          ...
          newStandardAssetHoldings = accountInformation.standardAssetHoldings.filter(
            (assetHolding) => !standardAssets.some((value) => value.id === assetHolding.id)
          );
