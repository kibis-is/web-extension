  ...
  const startStream = async () => {
    ...
    try {
      ...
      if ((error as DOMException).name === 'NotAllowedError') {
        setError(new ScreenCaptureNotAllowedError('screen capture was denied permission'));
        setLoading(false);

        return;
      }

      setError(new ScreenCaptureError((error as DOMException).name, error.message));
    }
  };
  const stopStream = () => {
