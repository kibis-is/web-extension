...
// errors
import { BaseExtensionError, InvalidPasswordError, MalformedDataError } from '@common/errors';

// managers
import PasswordManager from '@provider/managers/PasswordManager';

// selectors
import { useSelectLogger, useSelectPasskeysEnabled } from '@provider/selectors';
...
export default function useChangePassword(): IState {
  ...
  // states
  const [encryptionProgressState, setEncryptionProgressState] = useState<IEncryptionState[]>([]);
  ...
  const changePasswordAction = async ({
    ...
    if (currentPassword === newPassword) {
      logger.debug(`${_hookName}#${_functionName}: passwords match, ignoring update`);
    ...
    logger?.debug(`${_hookName}#${_functionName}: re-encrypted password tag "${passwordTag.id}"`);

    // only re-encrypt the keys if the passkey is not enabled
    if (!passkeyEnabled) {
      logger?.debug(`${_hookName}#${_functionName}: re-encrypting private keys`);
    ...
    logger?.debug(`${_hookName}#${_functionName}: successfully changed password`);
