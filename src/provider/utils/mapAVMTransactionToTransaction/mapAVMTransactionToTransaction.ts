...
// types
import type { IBaseTransaction, ITransactions, TAVMTransaction } from '@provider/types';
...
export default function mapAVMTransactionToTransaction(avmTransaction: TAVMTransaction): ITransactions {
  ...
  switch (avmTransaction['tx-type']) {
    case 'acfg':
      return parseAssetConfigTransaction(avmTransaction['asset-config-transaction'], baseTransaction);
    case 'afrz':
      return parseAssetFreezeTransaction(avmTransaction['asset-freeze-transaction'], baseTransaction);
    case 'appl':
    ...
    case 'axfer':
      return parseAssetTransferTransaction(avmTransaction['asset-transfer-transaction'], baseTransaction);
    case 'keyreg':
      return parseKeyRegistrationTransaction(avmTransaction['keyreg-transaction'], baseTransaction);
    case 'pay':
      return parsePaymentAndReKeyTransaction(avmTransaction['payment-transaction'], baseTransaction);
    default:
      break;
  }
