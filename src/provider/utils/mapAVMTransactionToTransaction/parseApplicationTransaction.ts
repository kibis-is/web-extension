...
export default function parseApplicationTransaction(
  ...
  const applicationId: string | null =
    algorandApplicationTransaction['application-id'] > 0
      ? new BigNumber(String(algorandApplicationTransaction['application-id'] as bigint)).toFixed()
      : null;
  ...
  return {
    ...
    clearStateProgram: algorandApplicationTransaction['clear-state-program'] || null,
    ...
    innerTransactions: innerTransactions?.map(mapAVMTransactionToTransaction) || null,
    type,
  };
}
