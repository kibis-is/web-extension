 ...
 */
export default function createIconFromDataUri(dataUri: string, props?: IconProps): ReactElement | null {
  ...
  document = parser.parseFromString(sanitize(svg, { USE_PROFILES: { svg: true, svgFilters: true } }), mimeType); // sanitize for any nastiness
  viewBox = document.getElementsByTagName('svg').item(0)?.getAttribute('viewBox') || undefined;

  return (
    <Icon viewBox={viewBox} {...props}>
      {Array.from(document.getElementsByTagName('path')).map((value, index) => (
        <path d={value.getAttribute('d') || undefined} fill="currentColor" key={`path-${index}`} />
      ))}
    </Icon>
  );
}
