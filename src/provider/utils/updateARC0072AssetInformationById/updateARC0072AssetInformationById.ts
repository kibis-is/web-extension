...
// types
import type { IARC0072Asset, IARC0072AssetInformation, IUpdateAssetInformationByIdOptions } from '@provider/types';
...
export default async function updateARC0072AssetInformationById({
  ...
  try {
    ...
    return mapARC0072AssetFromARC0072AssetInformation(id, assetInformation, false);
  } catch (error) {
    logger?.error(
