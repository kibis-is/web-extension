 ...
 */
export default function calculateMaxTransactionAmount({ account, asset, network }: IOptions): BigNumber {
  const accountInformation: IAccountInformation | null =
    account.networkInformation[convertGenesisHashToHex(network.genesisHash).toUpperCase()] || null;
  ...
  switch (asset.type) {
    case AssetTypeEnum.ARC0200:
      assetHolding = accountInformation.arc200AssetHoldings.find((value) => value.id === asset.id) || null;
    ...
    case AssetTypeEnum.Standard:
      assetHolding = accountInformation.standardAssetHoldings.find((value) => value.id === asset.id) || null;
