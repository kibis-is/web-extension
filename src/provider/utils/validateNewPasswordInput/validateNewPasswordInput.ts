// constants
import { PASSWORD_MIN_LENGTH, PASSWORD_MIN_SCORE } from '@provider/constants';
...
export default function validateNewPasswordInput({
  ...
  if (value.length <= 0 && required) {
    return field ? t<string>('errors.inputs.requiredWithLabel', { name: field }) : t<string>('errors.inputs.required');
  }
