...
// constants
import { ARC_0300_SCHEME, EXPORT_ACCOUNT_PAGE_LIMIT } from '@provider/constants';
 ...
 */
export default function createAccountImportURIs({ accounts }: IOptions): string[] {
  const totalPages: number = Math.ceil(accounts.length / EXPORT_ACCOUNT_PAGE_LIMIT);
  ...
  if (totalPages <= 1) {
    ...
    sortedAccounts.forEach(({ name, privateKey }) => {
      ...
      queryParams.append(ARC0300QueryEnum.PrivateKey, encodeBase64URLSafe(privateKey));
    });

    return [`${ARC_0300_SCHEME}://${ARC0300AuthorityEnum.Account}/${ARC0300PathEnum.Import}?${queryParams.toString()}`];
  }
  ...
  return Array.from<void, string>({ length: totalPages }, (_, index) => {
    ...
    accountsSlice = sortedAccounts.slice(start, start + EXPORT_ACCOUNT_PAGE_LIMIT);
    queryParams = new URLSearchParams();
    ...
    queryParams.append(ARC0300QueryEnum.Page, `${String(index)}:${String(totalPages)}`);
