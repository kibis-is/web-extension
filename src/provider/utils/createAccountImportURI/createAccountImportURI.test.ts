...
// constants
import { EXPORT_ACCOUNT_PAGE_LIMIT, ARC_0300_SCHEME } from '@provider/constants';
...
// types
import type { IARC0300AccountImportSchema, IARC0300PaginationQueryItem } from '@provider/types';
 ...
 */
function assertChecksumMatches(schemas: (IARC0300AccountImportSchema | null)[]): void {
  const match = schemas.every(
    (value, _, array) => value?.query[ARC0300QueryEnum.Checksum] === array[0]?.query[ARC0300QueryEnum.Checksum]
  );
 ...
 */
function assertSchemaWithoutQueryParams(schema: IARC0300AccountImportSchema | null): void {
...
describe(`${__dirname}#createAccountImportURI`, () => {
  ...
  describe('when there is a single account', () => {
    it('should return a schema for a single unnamed account', () => {
      ...
      expect(schema?.query[ARC0300QueryEnum.PrivateKey][0]).toBe(encodeBase64URLSafe(keyPair.privateKey()));
    });
  ...
  describe('when there is multiple accounts under the account limit', () => {
    it('should return a schema for multiple unnamed accounts', () => {
      // arrange
      const accounts: IExportAccount[] = Array.from({ length: EXPORT_ACCOUNT_PAGE_LIMIT - 1 }, () => ({
      ...
      expect(schema?.query[ARC0300QueryEnum.Name]).toHaveLength(0);
      expect(schema?.query[ARC0300QueryEnum.PrivateKey]).toHaveLength(accounts.length);
    ...
    it('should return a schema for multiple named accounts', () => {
      ...
      expect(schema?.query[ARC0300QueryEnum.Name]).toHaveLength(accounts.length);
      schema?.query[ARC0300QueryEnum.Name]?.forEach((value, index) => expect(value).toBe(accounts[index].name));
      expect(schema?.query[ARC0300QueryEnum.PrivateKey]).toHaveLength(accounts.length);
      schema?.query[ARC0300QueryEnum.PrivateKey].forEach((value, index) =>
        expect(value).toBe(encodeBase64URLSafe(accounts[index].privateKey))
      );
    ...
    it('should return a schema for multiple named and unnamed accounts', () => {
      // arrange
      const accounts: IExportAccount[] = Array.from({ length: EXPORT_ACCOUNT_PAGE_LIMIT - 1 }, (_, index) => ({
        privateKey: Ed21559KeyPair.generate().privateKey(),
        // only add evens
        ...(index % 2 === 0 && {
          name: encodeHex(randomBytes(16)), // 32 byte string max
      ...
      expect(schema?.query[ARC0300QueryEnum.Name]).toHaveLength(namedAccounts.length);
      schema?.query[ARC0300QueryEnum.Name]?.forEach((value, index) => expect(value).toBe(namedAccounts[index].name));
      expect(schema?.query[ARC0300QueryEnum.PrivateKey]).toHaveLength(accounts.length);
      schema?.query[ARC0300QueryEnum.PrivateKey].forEach((value, index) =>
        expect(value).toBe(encodeBase64URLSafe(sortedAccounts[index].privateKey))
      );
    });
  });
  ...
  describe('when there is multiple accounts over the account limit', () => {
    it('should return a schema for multiple unnamed accounts', () => {
      ...
      // assert
      expect(result).toHaveLength(Math.ceil(accounts.length / EXPORT_ACCOUNT_PAGE_LIMIT));
      ...
      schemas.forEach((value) => {
        ...
        if (pagination) {
          expectedLastPageItems = accounts.length - EXPORT_ACCOUNT_PAGE_LIMIT * (pagination.total - 1);
          ...
          expect(value?.query[ARC0300QueryEnum.PrivateKey]).toHaveLength(
            pagination.page < pagination.total - 1 ? EXPORT_ACCOUNT_PAGE_LIMIT : expectedLastPageItems
          );
        }
      });
    });
