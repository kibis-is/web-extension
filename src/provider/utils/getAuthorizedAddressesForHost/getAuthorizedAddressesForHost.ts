 ...
 */
export default function getAuthorizedAddressesForHost(host: string, sessions: ISession[]): string[] {
  return sessions
    .filter((value) => value.host === host)
    .reduce<string[]>(
      (acc, session) => [
        ...acc,
        ...session.authorizedAddresses.filter((address) => !acc.some((value) => address === value)), // get only unique addresses
      ],
      []
    );
}
