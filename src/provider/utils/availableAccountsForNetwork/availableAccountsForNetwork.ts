 ...
 */
export default function availableAccountsForNetwork({ accounts, network }: IOptions): IAccountWithExtendedProps[] {
  return accounts.filter((value, _, array) => {
    const accountInformation = AccountRepository.extractAccountInformationForNetwork(value, network);
