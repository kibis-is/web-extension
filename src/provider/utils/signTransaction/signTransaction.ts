...
// types
import type { IAccountInformation, IAccountWithExtendedProps } from '@provider/types';
...
export default async function signTransaction({
  ...
  const base64EncodedGenesisHash = encodeBase64(unsignedTransaction.genesisHash);
  const network = networks.find((value) => value.genesisHash === base64EncodedGenesisHash) || null;
  const signerAddress = convertPublicKeyToAVMAddress(unsignedTransaction.from.publicKey);
  let _error: string;
  ...
  logger?.debug(`${_functionName}: signing transaction "${unsignedTransaction.txID()}"`);

  if (!network) {
    _error = `network "${unsignedTransaction.genesisID}" for transaction "${unsignedTransaction.txID()}" not supported`;
  ...
  account =
    accounts.find((value) => value.publicKey === AccountRepository.encode(unsignedTransaction.from.publicKey)) || null;
  ...
  accountInformation = AccountRepository.extractAccountInformationForNetwork(account, network);
    ...
    authAccount =
      authAccounts.find(
        (value) =>
          ...
          value.publicKey === AccountRepository.encode(convertAVMAddressToPublicKey(accountInformation.authAddress))
      ) || null;
  ...
  logger?.debug(`${_functionName}: decrypting private key using "${encryptionOptions.type}" encryption method`);
