 ...
 */
export default function isReKeyedAuthAccountAvailable({ accounts, authAddress }: IOptions): boolean {
  const reKeyedAccount = accounts.find(
    (value) => convertPublicKeyToAVMAddress(AccountRepository.decode(value.publicKey)) === authAddress
  );
