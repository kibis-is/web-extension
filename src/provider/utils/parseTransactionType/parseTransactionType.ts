...
export default function parseTransactionType(
  ...
  if (encodedTransaction.type === 'afrz') {
    return encodedTransaction.afrz ? TransactionTypeEnum.AssetFreeze : TransactionTypeEnum.AssetUnfreeze;
  }
  ...
  if (encodedTransaction.type === 'axfer') {
    ...
    if (network && sender) {
      accountInformation = AccountRepository.extractAccountInformationForNetwork(sender, network);
      ...
      if (
        ...
        !accountInformation?.standardAssetHoldings.find((value) => value.id === String(encodedTransaction.xaid)) &&
