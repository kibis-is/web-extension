...
// types
import type { IPasskeyCredential, IPrivateKey } from '@provider/types';
...
export default async function fetchDecryptedKeyPairFromStorageWithPasskey({
  ...
  const _passkeyCredentialRepository = passkeyCredentialRepository || new PasskeyCredentialRepository();
  ...
  _publicKey = typeof publicKey !== 'string' ? PrivateKeyRepository.encode(publicKey) : publicKey; // encode the public key if it isn't already
  privateKeyItem = await _privateKeyRepository.fetchByPublicKey(_publicKey);

  if (!privateKeyItem) {
    logger?.debug(`${_functionName}: no private key stored for public key "${_publicKey}"`);
  ...
  logger?.debug(`${_functionName}: decrypting private key for public key "${_publicKey}"`);
