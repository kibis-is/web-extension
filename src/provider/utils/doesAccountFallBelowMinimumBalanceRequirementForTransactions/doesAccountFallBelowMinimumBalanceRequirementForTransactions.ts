...
export default function doesAccountFallBelowMinimumBalanceRequirementForTransactions({
  ...
  const _functionName: string = 'doesAccountFallBelowMinimumBalanceRequirementForTransactions';
  ...
  minimumBalanceRequirementForAccount = new BigNumber(accountInformation.minAtomicBalance);

  // number of transactions * the minimum transaction fee
  transactionFees = new BigNumber(network.minFee).multipliedBy(transactions.length);
  ...
  remainderBalanceAfterCost = new BigNumber(accountInformation.atomicBalance).minus(
    transactionFees.plus(payTransactionsCost)
  );
