 ...
 */
export default function groupTransactions(transactions: Transaction[]): Transaction[][] {
  return transactions.reduce<Transaction[][]>((acc, currentValue) => {
    ...
    previousGroupIndex = acc.findIndex((value) =>
      value[0]?.group && currentValue.group ? encodeBase64(value[0].group) === encodeBase64(currentValue.group) : false
    );
    ...
    return acc.map((value, index) => (previousGroupIndex === index ? [...value, currentValue] : value));
  }, []);
}
