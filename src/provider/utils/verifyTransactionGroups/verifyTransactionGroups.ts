 ...
 */
export default function verifyTransactionGroups(transactions: Transaction[]): boolean {
  ...
  return groupsOfTransactions
    .reduce<boolean[]>((acc, currentValue) => {
      ...
      return [
        ...
        currentValue.every((value) => value.group && encodeBase64(value.group) === computedEncodedGroupId),
      ];
    }, [])
    .every((value) => value);
}
