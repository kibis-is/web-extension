...
// contracts
import ARC0200Contract, { ARC0200MethodEnum } from '@provider/contracts/ARC0200Contract';

// enums
import { TransactionTypeEnum } from '@provider/enums';

// types
import type { IApplicationTransaction, IARC0200AssetTransferTransaction } from '@provider/types';
...
export default function parseARC0200Transaction(
  ...
  if (!transaction.applicationArgs || transaction.applicationArgs.length <= 0 || !transaction.applicationId) {
    return null;
  }
  ...
  // if it is a transfer
  if (isByteArrayEqual(method, abiContract.getMethodByName(ARC0200MethodEnum.Transfer).getSelector())) {
    return {
      ...transaction,
      amount: ARC0200Contract.formatBase64EncodedUintArg(transaction.applicationArgs[2], 256).toFixed(),
      receiver: ARC0200Contract.formatBase64EncodedAddressArg(transaction.applicationArgs[1]),
