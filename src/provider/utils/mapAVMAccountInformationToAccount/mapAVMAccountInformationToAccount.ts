...
// types
import type { IAccountInformation, IAVMAccountInformation } from '@provider/types';
...
export default function mapAVMAccountInformationToAccountInformation(
  ...
  return {
    ...accountInformation,
    atomicBalance: new BigNumber(String(avmAccountInformation.amount as bigint)).toFixed(),
    authAddress: avmAccountInformation['auth-addr'] || null,
    minAtomicBalance: new BigNumber(String(avmAccountInformation['min-balance'] as bigint)).toFixed(),
