 ...
 */
export default function authorizedAccountsForHost({ accounts, host, sessions }: IOptions): IAccountWithExtendedProps[] {
  return sessions
        ...
        ...accounts.filter(
          (account) =>
            ...
            currentValue.authorizedAddresses.some(
              (value) => value === convertPublicKeyToAVMAddress(AccountRepository.decode(account.publicKey))
