// constants
import { TINYMAN_ASA_LIST_API_URL } from '@provider/constants';

// types
import { IBaseOptions } from '@common/types';
import { INetwork, ITinyManAssetResponse } from '@provider/types';
...
export default async function fetchVerifiedStandardAssetList({
  ...
  if (network.genesisHash !== 'wGHE2Pwdvd7S12BL5FaOP20EGYesN73ktiC1qzkkit8=') {
    logger && logger.debug(`${fetchVerifiedStandardAssetList.name}(): no asset list defined, skipping`);
  ...
  try {
    ...
    logger && logger.error(`${fetchVerifiedStandardAssetList.name}(): ${error.message}`);
