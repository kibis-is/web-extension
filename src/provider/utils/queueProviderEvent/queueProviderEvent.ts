...
export default async function queueProviderEvent({
  ...
  const _eventQueueRepository = eventQueueRepository || new EventQueueRepository();
  const _functionName = 'sendProviderEvent';
  const isInitialized = await isProviderInitialized();
  const mainAppWindows = await _appWindowRepository.fetchByType(AppTypeEnum.MainApp);
  ...
  logger?.debug(`${_functionName}: saving event "${event.type}" to event queue`);
  ...
  logger?.debug(`${_functionName}: main app window not open, opening background app window for "${event.type}" event`);
