...
// enums
import { ARC0300AssetTypeEnum, ARC0300AuthorityEnum, ARC0300PathEnum, ARC0300QueryEnum } from '@provider/enums';

// types
import type { IARC0300AssetAddSchema } from '@provider/types';
...
describe(`${__dirname}#parseARC0300AssetAddSchema()`, () => {
  ...
  const genesisHashURLSafe: string = encodeBase64URLSafe(decodeBase64(networks[0].genesisHash));
  ...
  it('should return null if no the asset id path was provided', () => {
    ...
    // act
    const result: IARC0300AssetAddSchema | null = parseURIToARC0300Schema<IARC0300AssetAddSchema>(uri, options);
  ...
  it('should return null if the genesis hash is not supported', () => {
    ...
    // act
    const result: IARC0300AssetAddSchema | null = parseURIToARC0300Schema<IARC0300AssetAddSchema>(uri, {
      ...options,
  ...
  it('should return a valid schema', () => {
    ...
    expect(result.query[ARC0300QueryEnum.GenesisHash]).toBe(networks[0].genesisHash);
    expect(result.query[ARC0300QueryEnum.Type]).toEqual(ARC0300AssetTypeEnum.ARC0200);
  });
});
