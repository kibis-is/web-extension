// constants
import { ARC_0026_SCHEME, ARC_0300_SCHEME } from '@provider/constants';

// enums
import { ARC0300AuthorityEnum, ARC0300PathEnum } from '@provider/enums';

// types
import type { IARC0300BaseSchema } from '@provider/types';
...
export default function parseURIToARC0300Schema<Schema = IARC0300BaseSchema>(
  ...
  const [scheme, authorityPathsAndQuery] = uri.split(':').filter((value) => value);
  ...
  if (scheme !== ARC_0300_SCHEME && scheme !== ARC_0026_SCHEME) {
    logger?.debug(`${_functionName}: not an arc0300 or arc0026 scheme, found "${scheme}"`);

    return null;
  }

  const [authorityAndPaths, query] = authorityPathsAndQuery.split('?').filter((value) => value);
  const [authority, ...paths] = authorityAndPaths.split('/').filter((value) => value);
  ...
  switch (authority) {
    case ARC0300AuthorityEnum.Account:
      ...
      if (paths[0] === ARC0300PathEnum.Import) {
        return parseARC0300AccountImportSchema(scheme, paths, new URLSearchParams(query), { logger }) as Schema;
      }

      break;
    case ARC0300AuthorityEnum.Asset:
      if (paths[0] === ARC0300PathEnum.Add) {
        return parseARC0300AssetAddSchema(scheme, paths, new URLSearchParams(query), {
          logger,
          supportedNetworks,
        }) as Schema;
      }
    ...
    case ARC0300AuthorityEnum.Transaction:
      if (paths[0] === ARC0300PathEnum.Send) {
        return parseARC0300TransactionSendSchema(scheme, paths, new URLSearchParams(query), {
          logger,
          supportedNetworks,
        }) as Schema;
      }
