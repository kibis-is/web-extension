...
export default function parseARC0300KeyRegistrationTransactionSendSchema(
  ...
  options: IOptions
): IARC0300OfflineKeyRegistrationTransactionSendSchema | IARC0300OnlineKeyRegistrationTransactionSendSchema | null {
  const _functionName = 'parseARC0300KeyRegistrationTransactionSendSchema';
  const logger = options?.logger;
  const commonQuery = parseARC0300CommonTransactionSendQuery(searchParams, options);
  ...
  const voteKeyDilutionParam = searchParams.get(ARC0300QueryEnum.VoteKeyDilution);
  ...
  if (!voteFirstParam || !isNumericString(voteFirstParam)) {
    logger?.debug(`${_functionName}: "votefirst" param is not found or not valid`);
  ...
  if (!voteKeyDilutionParam || !isNumericString(voteKeyDilutionParam)) {
    logger?.debug(`${_functionName}: "votekeydilution" param is not found or not valid`);

    return null;
  }

  if (!voteLastParam || !isNumericString(voteLastParam)) {
    logger?.debug(`${_functionName}: "votelast" param is not found or not valid`);
  ...
  return {
    ...
    query: {
      ...commonQuery,
      [ARC0300QueryEnum.SelectionKey]: encodeBase64(decodeBase64URLSafe(selectionKeyParam)),
      [ARC0300QueryEnum.StateProofKey]: encodeBase64(decodeBase64URLSafe(stateProofKeyParam)),
      [ARC0300QueryEnum.VoteFirst]: voteFirstParam,
      [ARC0300QueryEnum.VoteKey]: encodeBase64(decodeBase64URLSafe(voteKeyParam)),
