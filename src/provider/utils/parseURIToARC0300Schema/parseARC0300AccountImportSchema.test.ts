...
// types
import type { IARC0300AccountImportSchema } from '@provider/types';
...
describe(`${__dirname}#parseARC0300AccountImportSchema`, () => {
  ...
  it('should return null if no private key param is provided', () => {
    ...
    // act
    const result = parseURIToARC0300Schema<IARC0300AccountImportSchema>(uri, options);
  ...
  it('should return a valid schema', () => {
    ...
    queryParams.append(ARC0300QueryEnum.PrivateKey, encodeBase64URLSafe(keyPair.privateKey()));
    ...
    expect(result.query[ARC0300QueryEnum.PrivateKey].includes(encodeBase64URLSafe(keyPair.privateKey()))).toBe(true);
  });
