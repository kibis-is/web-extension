...
describe(`${__dirname}#parseARC0300KeyRegistrationTransactionSendSchema()`, () => {
  ...
  const stateProofKey = 'RpUpNWfZMjZ1zOOjv3MF2tjO714jsBt0GKnNsw0ihJ4HSZwci+d9zvUi3i67LwFUJgjQ5Dz4zZgHgGduElnmSA==';
  ...
  it('should return null if the "selectionkey" param is missing', () => {
    ...
    const query = new URLSearchParams({
      ...commonQueryFragment,
      [ARC0300QueryEnum.StateProofKey]: encodeBase64URLSafe(decodeBase64(stateProofKey)),
  ...
  it('should return null if the "stateproofkey" param is missing', () => {
    ...
    const query = new URLSearchParams({
      ...commonQueryFragment,
      [ARC0300QueryEnum.SelectionKey]: encodeBase64URLSafe(decodeBase64(selectionKey)),
      [ARC0300QueryEnum.VoteFirst]: voteFirst,
      [ARC0300QueryEnum.VoteKey]: encodeBase64URLSafe(decodeBase64(voteKey)),
      [ARC0300QueryEnum.VoteKeyDilution]: voteKeyDilution,
      [ARC0300QueryEnum.VoteLast]: voteLast,
  ...
  it('should return a valid online schema', () => {
    ...
    const uri = `${baseURI}?${query.toString()}`;
    // act
    const result = parseURIToARC0300Schema<IARC0300OnlineKeyRegistrationTransactionSendSchema>(uri, options);
    ...
    expect(result.query[ARC0300QueryEnum.VoteKeyDilution]).toBe(voteKeyDilution);
    expect(result.query[ARC0300QueryEnum.VoteLast]).toBe(voteLast);
  });

  it('should return a valid offline schema', () => {
    // arrange
    const uri = `${baseURI}?${new URLSearchParams(commonQueryFragment).toString()}`;
    // act
    const result = parseURIToARC0300Schema<IARC0300OfflineKeyRegistrationTransactionSendSchema>(uri, options);
