...
export default function parseARC0300AssetAddSchema(
  ...
  if (!supportedNetworks.find((value) => value.genesisHash === genesisHash)) {
    logger?.debug(`${_functionName}: network with genesis hash "${genesisHash}" not supported`);
  ...
  switch (typeParam) {
    case ARC0300AssetTypeEnum.ARC0200:
      ...
      break;
    default:
      logger?.debug(`${_functionName}: unknown type param "${typeParam}" for method "${ARC0300PathEnum.Add}"`);
