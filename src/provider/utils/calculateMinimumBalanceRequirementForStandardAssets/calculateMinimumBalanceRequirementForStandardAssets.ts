...
export default function calculateMinimumBalanceRequirementForStandardAssets({
  ...
  let accountMinimumBalance: BigNumber = new BigNumber(MINIMUM_BALANCE_REQUIREMENT);
  ...
  if (numOfStandardAssets < 0) {
    minimumBalanceToRemoveInAtomicUnits = new BigNumber(MINIMUM_BALANCE_REQUIREMENT).multipliedBy(
      Math.abs(roundedNumberOfAssets)
    );
    accountMinimumBalance = accountMinimumBalance.minus(minimumBalanceToRemoveInAtomicUnits);
  ...
  return accountMinimumBalance.plus(new BigNumber(MINIMUM_BALANCE_REQUIREMENT).multipliedBy(roundedNumberOfAssets));
}
