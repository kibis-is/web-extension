// constants
import { VESTIGE_FI_API_URL } from '@provider/constants';

// types
import type { IBaseOptions } from '@common/types';
import type { INetwork, IVestigeFiAssetResponse } from '@provider/types';
...
export default async function fetchAssetVerification(
  ...
  logger &&
    logger.debug(`${fetchAssetVerification.name}: getting verified status for "${id}" on "${network.genesisId}"`);

  // vestige only verifies mainnet
  // TODO: use custom service?
  if (network.genesisHash !== 'wGHE2Pwdvd7S12BL5FaOP20EGYesN73ktiC1qzkkit8=') {
    logger && logger.debug(`${fetchAssetVerification.name}(): asset "${id}" not on mainnet, skipping`);
  ...
  try {
    ...
    logger && logger.error(`${fetchAssetVerification.name}(): ${error.message}`);
