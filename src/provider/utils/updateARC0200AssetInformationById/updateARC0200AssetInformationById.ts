...
// types
import type { IARC0200AssetInformation, IARC0200Asset, IUpdateAssetInformationByIdOptions } from '@provider/types';
...
export default async function updateARC0200AssetInformationById({
  ...
  try {
    ...
    return mapARC0200AssetFromARC0200AssetInformation(id, assetInformation, null, false);
  } catch (error) {
    logger?.error(
