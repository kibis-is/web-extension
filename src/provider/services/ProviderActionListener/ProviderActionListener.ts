...
// constants
import { CREDENTIAL_LOCK_ALARM } from '@provider/constants';

// enums
import { AppTypeEnum, ARC0300AuthorityEnum, ARC0300PathEnum, EventTypeEnum } from '@provider/enums';
...
export default class ProviderActionListener extends BaseListener {
  ...
  private async _getMainWindow(includeTabs: boolean = false): Promise<Windows.Window | null> {
    const mainAppWindows = await this._appWindowRepository.fetchByType(AppTypeEnum.MainApp);
  ...
  private async _handleCredentialLockActivated(): Promise<void> {
    ...
    this._logger?.debug(`${ProviderActionListener.displayName}#${__function}: removed decrypted private keys`);

    // send a message to the popups to lock the screen
    await browser.runtime.sendMessage(new ProviderCredentialLockActivatedMessage());
  }

  private async _onAlarm(alarm: Alarms.Alarm): Promise<void> {
    const __function = 'onAlarm';

    this._logger?.debug(`${ProviderActionListener.displayName}#${__function}: alarm "${alarm.name}" fired`);
  ...
  private async _onExtensionClick(): Promise<void> {
    ...
    this._logger?.debug(`${ProviderActionListener.displayName}#${__function}: browser extension clicked`);
  ...
  private async _onOmniboxInputEntered(text: string): Promise<void> {
    ...
    this._logger?.debug(`${ProviderActionListener.displayName}#${__function}: received omnibox input "${text}"`);
      ...
      switch (arc0300Schema.authority) {
        case ARC0300AuthorityEnum.Transaction:
          ...
          if (arc0300Schema.paths[0] === ARC0300PathEnum.Send) {
            switch ((arc0300Schema as TARC0300TransactionSendSchemas).query.type) {
  ...
  private async _onWindowRemove(windowId: number): Promise<void> {
    ...
    if (appWindow) {
      this._logger?.debug(`${ProviderActionListener.displayName}#${__function}: removed "${appWindow.type}" window`);
  ...
  private async _restartCredentialLockAlarm(): Promise<void> {
    ...
    if (
      ...
      await this._credentialLockService.restartAlarm(settings.security.credentialLockTimeoutDuration);
  ...
  public startListening(): void {
    ...
    browser.omnibox.onInputEntered.addListener(this._onOmniboxInputEntered.bind(this));
  ...
  public stopListening() {
    ...
    browser.omnibox.onInputEntered.removeListener(this._onOmniboxInputEntered.bind(this));
    browser.runtime.onInstalled.removeListener(this._onInstalled.bind(this));
    browser.windows.onFocusChanged.removeListener(this._onFocusChanged.bind(this));
