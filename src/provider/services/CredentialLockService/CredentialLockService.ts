...
// utils
import convertMillisecondsToMinutes from '@provider/utils/convertMillisecondsToMinutes';
...
export default class CredentialLockService {
  ...
  public async clearAlarm(): Promise<void> {
    ...
    this._logger?.debug(`${CredentialLockService.name}#${_functionName}: clearing credential lock alarm`);

    await browser.alarms.clear(CREDENTIAL_LOCK_ALARM);
  }

  public async createAlarm(timeoutInMilliseconds: number): Promise<Alarms.Alarm | null> {
  ...
  public async restartAlarm(timeoutInMilliseconds: number): Promise<Alarms.Alarm | null> {
