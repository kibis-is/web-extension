...
// constants
import { HEARTBEAT_ALARM, HEARTBEAT_DURATION } from '@provider/constants';
...
export default class HeartbeatService {
  ...
  public async clearAlarm(): Promise<void> {
    ...
    this.logger?.debug(`${HeartbeatService.name}#${_functionName}: clearing heartbeat alarm`);
  ...
  public async createOrGetAlarm(): Promise<Alarms.Alarm | null> {
    ...
    const periodInMinutes: number = convertMillisecondsToMinutes(HEARTBEAT_DURATION);
