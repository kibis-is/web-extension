// constants
import { ARC0072_ASSETS_KEY_PREFIX } from '@provider/constants';
...
export default class ARC0072AssetRepository extends BaseRepository {
  ...
  private _createItemKey(genesisHash: string): string {
    return `${ARC0072_ASSETS_KEY_PREFIX}${convertGenesisHashToHex(genesisHash)}`;
  }
   ...
   * @public
   */
  public async fetchByGenesisHash(genesisHash: string): Promise<IARC0072Asset[]> {
    const items = await this._fetchByKey<IARC0072Asset[]>(this._createItemKey(genesisHash));
   ...
   * @public
   */
  public async saveByGenesisHash({ genesisHash, items }: ISaveOptions): Promise<IARC0072Asset[]> {
