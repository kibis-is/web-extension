// constants
import { STANDARD_ASSETS_KEY_PREFIX } from '@provider/constants';
...
export default class StandardAssetRepository extends BaseRepository {
  ...
  private _createItemKey(genesisHash: string): string {
    return `${STANDARD_ASSETS_KEY_PREFIX}${convertGenesisHashToHex(genesisHash)}`;
  }
   ...
   * @public
   */
  public async fetchByGenesisHash(genesisHash: string): Promise<IStandardAsset[]> {
    const items = await this._fetchByKey<IStandardAsset[]>(this._createItemKey(genesisHash));
   ...
   * @public
   */
  public async saveByGenesisHash({ genesisHash, items }: ISaveOptions): Promise<IStandardAsset[]> {
