...
// constants
import { NETWORKS_ITEM_KEY } from '@provider/constants';

// repositories
import BaseRepository from '@provider/repositories/BaseRepository';

// types
import type { INetwork, INetworkWithTransactionParams, ITransactionParams } from '@provider/types';
...
export default class NetworksRepository extends BaseRepository {
  ...
  public static mapWithDefaultTransactionParams(value: INetwork): INetworkWithTransactionParams {
    return {
   ...
   * @private
   */
  private _deserialize(value: ISerializableNetworkWithTransactionParams): INetworkWithTransactionParams {
    return {
      ...value,
      arc0072Indexers: networks.find((_value) => _value.genesisHash === value.genesisHash)?.arc0072Indexers || [],
      blockExplorers: networks.find((_value) => _value.genesisHash === value.genesisHash)?.blockExplorers || [],
      enVoi: networks.find((_value) => _value.genesisHash === value.genesisHash)?.enVoi || null,
      nftExplorers: networks.find((_value) => _value.genesisHash === value.genesisHash)?.nftExplorers || [],
      scsIndexers: networks.find((_value) => _value.genesisHash === value.genesisHash)?.scsIndexers || [],
    };
  }

  private async _fetchSerializedFromStorage(): Promise<ISerializableNetworkWithTransactionParams[]> {
    return (await this._fetchByKey<ISerializableNetworkWithTransactionParams[]>(NETWORKS_ITEM_KEY)) || [];
  }
   ...
   * @private
   */
  private _serialize(value: INetworkWithTransactionParams): ISerializableNetworkWithTransactionParams {
    return {
      ...value,
  ...
  public async fetchAll(): Promise<INetworkWithTransactionParams[]> {
    ...
    return items.map((value) =>
      this._deserialize({
        ...networks.find(({ genesisHash }) => genesisHash === value.genesisHash), // update with any new fields from the config
   ...
   * @public
   */
  public async save(item: INetworkWithTransactionParams): Promise<INetworkWithTransactionParams> {
    ...
    if (!serializedItems.find((value) => value.genesisHash === item.genesisHash)) {
   ...
   * @public
   */
  public async saveAll(items: INetworkWithTransactionParams[]): Promise<INetworkWithTransactionParams[]> {
    await this._save<ISerializableNetworkWithTransactionParams[]>({
