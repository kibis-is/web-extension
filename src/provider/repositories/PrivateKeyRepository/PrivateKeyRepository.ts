...
// constants
import { PRIVATE_KEY_ITEM_KEY_PREFIX } from '@provider/constants';
...
export default class PrivateKeyRepository extends BaseRepository {
   ...
   * @static
   */
  public static extractPrivateKeyFromSecretKey(secretKey: Uint8Array): Uint8Array {
  ...
  public static async upgrade({
    ...
    switch (encryptionCredentials.type) {
      case EncryptionMethodEnum.Passkey:
        decryptedPrivateKey = await PasskeyManager.decryptBytes({
          encryptedBytes: PrivateKeyRepository.decode(privateKeyItem.encryptedPrivateKey),
    ...
    if (!privateKeyItem.version || privateKeyItem.version <= 0) {
      decryptedPrivateKey = PrivateKeyRepository.extractPrivateKeyFromSecretKey(decryptedPrivateKey);
      ...
      _privateKeyItem = {
        ..._privateKeyItem,
        encryptedPrivateKey: PrivateKeyRepository.encode(encryptedPrivateKey),
        privateKey: _privateKeyItem.privateKey ? PrivateKeyRepository.encode(decryptedPrivateKey) : null,
      };
    }
  ...
  private _sanitize({
    ...
    if (passwordTagId && (!encryptionID || encryptionMethod !== EncryptionMethodEnum.Passkey)) {
  ...
  public async fetchAll(): Promise<IPrivateKey[]> {
    return await this._fetchByPrefixKey<IPrivateKey>(PRIVATE_KEY_ITEM_KEY_PREFIX);
  }
   ...
   * @public
   */
  public async fetchByPublicKey(publicKey: Uint8Array | string): Promise<IPrivateKey | null> {
    const _publicKey = typeof publicKey !== 'string' ? PrivateKeyRepository.encode(publicKey) : publicKey;
    const item = await this._fetchByKey<IPrivateKey>(this._createItemKey(_publicKey));
   ...
   * @public
   */
  public async removeByPublicKey(publicKey: Uint8Array | string): Promise<void> {
    const _publicKey = typeof publicKey !== 'string' ? PrivateKeyRepository.encode(publicKey) : publicKey;
