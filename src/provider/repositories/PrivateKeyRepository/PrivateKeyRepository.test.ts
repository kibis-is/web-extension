...
describe(PrivateKeyRepository.name, () => {
  ...
  beforeEach(() => {
    keyPair = Ed21559KeyPair.generateFromPrivateKey(
      new Uint8Array([
        133, 59, 225, 210, 192, 174, 46, 88, 148, 71, 152, 110, 196, 157, 235, 134, 145, 160, 3, 94, 85, 55, 128, 160,
        110, 43, 201, 246, 232, 78, 95, 18,
      ])
    );
  ...
  describe('upgrade', () => {
    it('should upgrade a legacy private key item (with no version) using password', async () => {
      ...
      expect(PrivateKeyRepository.encode(decryptedPrivateKey)).toBe(PrivateKeyRepository.encode(keyPair.privateKey()));
    });
