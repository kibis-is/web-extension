...
// types
import type { TStorageItemTypes } from '@provider/types';
...
export default class BaseRepository {
   ...
   * @protected
   */
  public async _fetchByKey<ItemType = TStorageItemTypes>(key: string): Promise<ItemType | null> {
   ...
   * @protected
   */
  public async _fetchByPrefixKey<ItemType = TStorageItemTypes>(keyPrefix: string): Promise<ItemType[]> {
    const allItems = await browser.storage.local.get();

    return Object.keys(allItems).reduce<ItemType[]>(
      (acc, key) => (key.startsWith(keyPrefix) ? [...acc, allItems[key] as ItemType] : acc),
      []
    );
  }
