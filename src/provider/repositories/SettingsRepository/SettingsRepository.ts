...
export default class SettingsRepository extends BaseRepository {
   ...
   * @private
   */
  private _sanitize({ advanced, appearance, general, privacy, security }: ISettings): ISettings {
    ...
    return {
      ...
      general: {
        ...
        selectedNetworkGenesisHash: general.selectedNetworkGenesisHash || genesisHash,
  ...
  public async fetch(): Promise<ISettings> {
    ...
    const mergedSettings = Object.keys(allItems).reduce<ISettings>((acc, value) => {
      switch (value) {
