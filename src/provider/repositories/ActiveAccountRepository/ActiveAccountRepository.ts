// constants
import { ACTIVE_ACCOUNT_DETAILS_KEY } from '@provider/constants';

// repositories
import BaseRepository from '@provider/repositories/BaseRepository';

// types
import type { IActiveAccountDetails } from '@provider/types';
...
export default class ActiveAccountRepository extends BaseRepository {
  ...
  public async fetch(): Promise<IActiveAccountDetails | null> {
    return await this._fetchByKey<IActiveAccountDetails>(ACTIVE_ACCOUNT_DETAILS_KEY);
  }
   ...
   */
  public async save(value: IActiveAccountDetails): Promise<IActiveAccountDetails> {
