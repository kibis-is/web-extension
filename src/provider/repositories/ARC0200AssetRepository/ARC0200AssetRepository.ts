// constants
import { ARC0200_ASSETS_KEY_PREFIX } from '@provider/constants';
...
export default class ARC0200AssetRepository extends BaseRepository {
  ...
  private _createItemKey(genesisHash: string): string {
    return `${ARC0200_ASSETS_KEY_PREFIX}${convertGenesisHashToHex(genesisHash)}`;
  }
   ...
   * @public
   */
  public async fetchByGenesisHash(genesisHash: string): Promise<IARC0200Asset[]> {
    const items = await this._fetchByKey<IARC0200Asset[]>(this._createItemKey(genesisHash));
   ...
   * @public
   */
  public async saveByGenesisHash({ genesisHash, items }: ISaveOptions): Promise<IARC0200Asset[]> {
