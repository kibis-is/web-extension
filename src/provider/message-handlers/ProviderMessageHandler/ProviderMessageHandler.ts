...
// enums
import { ExternalConfigMessageReferenceEnum, ProviderMessageReferenceEnum } from '@common/enums';
import { AppTypeEnum } from '@provider/enums';

// managers
import AppWindowManager from '@provider/managers/AppWindowManager';
...
export default class ProviderMessageHandler extends BaseListener {
  ...
  private async _handleFactoryResetMessage({ reference }: BaseProviderMessage): Promise<void> {
    const __function = '_handleFactoryResetMessage';
    let backgroundAppWindows: IAppWindow[];
    let mainAppWindows: IAppWindow[];

    this._logger?.debug(`${ProviderMessageHandler.displayName}#${__function}: message "${reference}" received`);

    backgroundAppWindows = await this._appWindowRepository.fetchByType(AppTypeEnum.BackgroundApp);
    mainAppWindows = await this._appWindowRepository.fetchByType(AppTypeEnum.MainApp);

    // remove the main app if it exists
    if (mainAppWindows.length > 0) {
      await Promise.all(mainAppWindows.map(async (value) => await browser.windows.remove(value.windowId)));
    }

    // remove the background apps if they exist
    if (backgroundAppWindows.length > 0) {
      await Promise.all(backgroundAppWindows.map(async (value) => await browser.windows.remove(value.windowId)));
    }
  ...
  private async _handleRegistrationCompletedMessage({ reference }: BaseProviderMessage): Promise<void> {
    ...
    mainAppWindows = await this._appWindowRepository.fetchByType(AppTypeEnum.MainApp);
    registrationAppWindows = await this._appWindowRepository.fetchByType(AppTypeEnum.RegistrationApp);
    ...
    if (registrationAppWindows.length > 0) {
      await Promise.all(registrationAppWindows.map(async (value) => await browser.windows.remove(value.windowId)));
    }
  ...
  private async _handleSettingsUpdatedMessage({ reference }: ProviderSessionsUpdatedMessage): Promise<void> {
  ...
  private async _handleThemeUpdatedMessage({ reference }: ProviderThemeUpdatedMessage): Promise<void> {
  ...
  private async _onProviderMessage(message: BaseProviderMessage): Promise<void> {
    switch (message.reference) {
      ...
      case ProviderMessageReferenceEnum.SettingsUpdated:
        return await this._handleSettingsUpdatedMessage(message as ProviderSessionsUpdatedMessage);
      case ProviderMessageReferenceEnum.ThemeUpdated:
        return await this._handleThemeUpdatedMessage(message as ProviderThemeUpdatedMessage);
      default:
        break;
    }
  }
   ...
   * @private
   */
  private async _sendMessageToMiddlewares(message: IBaseMessage): Promise<void> {
    ...
    await Promise.all(tabs.map(async ({ id }) => id && (await browser.tabs.sendMessage(id, message))));
  }
  ...
  public stopListening() {
    browser.runtime.onMessage.removeListener(this._onProviderMessage.bind(this));
  }
}
