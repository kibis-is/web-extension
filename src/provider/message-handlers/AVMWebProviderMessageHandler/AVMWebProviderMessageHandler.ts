...
import type { IAccount, IAccountWithExtendedProps, INetwork, ISession } from '@provider/types';

// utils
import authorizedAccountsForHost from '@provider/utils/authorizedAccountsForHost';
import convertPublicKeyToAVMAddress from '@common/utils/convertPublicKeyToAVMAddress';
import decodeUnsignedTransaction from '@provider/utils/decodeUnsignedTransaction';
import isNetworkSupportedFromSettings from '@provider/utils/isNetworkSupportedFromSettings';
...
import verifyTransactionGroups from '@provider/utils/verifyTransactionGroups';
...
export default class AVMWebProviderMessageHandler extends BaseMessageHandler {
  ...
  private async _fetchSessions(
    filterPredicate?: (value: ISession, index: number, array: ISession[]) => boolean
  ): Promise<ISession[]> {
  ...
  private async _handleDisableRequestMessage(
    ...
    if (message.payload.params.genesisHash) {
      network = networks.find((value) => value.genesisHash === message.payload.params?.genesisHash) || network;
    }

    // get the network if a genesis hash is present
    if (!network) {
      this._logger?.debug(`${AVMWebProviderMessageHandler.name}#${_functionName}: network not found`);

      // send the response to the web page (via the content script)
      return await this._sendResponseToMiddleware(
        new AVMWebProviderResponseMessage<IDisableResult>({
          error: new ARC0027NetworkNotSupportedError({
            genesisHashes: message.payload.params?.genesisHash ? [message.payload.params.genesisHash] : [],
            message: `no parameters supplied`,
            providerId: __PROVIDER_ID__,
          }),
          id: generateUUID(),
    ...
    sessions = await this._fetchSessions(
      (value) => value.host === message.payload.clientInfo.host && value.genesisHash === network?.genesisHash
    );

    // if session ids has been specified, filter the sessions
    if (message.payload.params.sessionIds && message.payload.params.sessionIds.length > 0) {
      sessions = sessions.filter((value) => message.payload.params?.sessionIds?.includes(value.id));
    }

    sessionIds = sessions.map((value) => value.id);

    this._logger?.debug(
      `${AVMWebProviderMessageHandler.name}#${_functionName}: removing sessions [${sessionIds
        .map((value) => `"${value}"`)
        .join(',')}] on host "${message.payload.clientInfo.host}" for network "${network.genesisId}"`
    );
    ...
    return await browser.runtime.sendMessage(new ProviderSessionsUpdatedMessage(sessions));
  }
    ...
    return await this._sendResponseToMiddleware(
      new AVMWebProviderResponseMessage<IDiscoverResult>({
        ...
        result: {
          ...
          icon: ICON_URI,
          name: __APP_TITLE__,
          networks: supportedNetworks.map(({ genesisHash, genesisId, methods }) => ({
            genesisHash,
            genesisId,
            methods,
          })),
  ...
  private async _handleEnableRequestMessage(
    ...
    if (message.payload.params?.genesisHash) {
      ...
      sessionFilterPredicate = (value) => value.genesisHash === message.payload.params?.genesisHash;
    }

    sessions = await this._fetchSessions(sessionFilterPredicate);
    session = sessions.find((value) => value.host === message.payload.clientInfo.host) || null;

    // if we have a session, update its use and return it
    if (session) {
      sessionNetwork = networks.find((value) => value.genesisHash === session?.genesisHash) || null;
      ...
      if (sessionNetwork) {
        ...
        return await this._sendResponseToMiddleware(
          new AVMWebProviderResponseMessage<IEnableResult>({
            ...
            requestID: message.id,
            result: {
              accounts: session.authorizedAddresses.map<IAVMProviderAccount>((address) => {
                const account: IAccount | null =
                  accounts.find(
                    (value) => convertPublicKeyToAVMAddress(AccountRepository.decode(value.publicKey)) === address
                  ) || null;
  ...
  private async _handleSignMessageRequestMessage(
    ...
    const filteredSessions = await this._fetchSessions((value) => value.host === message.payload.clientInfo.host);
    let _error: string;
    ...
    if (message.payload.params.signer) {
      signerAccount =
        authorizedAccounts.find(
          (value) =>
            convertPublicKeyToAVMAddress(AccountRepository.decode(value.publicKey)) === message.payload.params?.signer
        ) || null;

      if (!signerAccount || signerAccount.watchAccount) {
        _error = `"${message.payload.params.signer}" ${
          signerAccount?.watchAccount ? ` is a watch account` : `has not been authorized`
        }`;

        this._logger?.debug(`${AVMWebProviderMessageHandler.name}#${_functionName}: ${_error}`);
  ...
  private async _handleSignTransactionsRequestMessage(
    ...
    try {
      ...
      this._logger?.debug(`${AVMWebProviderMessageHandler.name}#${_functionName}: ${errorMessage}`);
    ...
    unsupportedTransactionsByNetwork = decodedUnsignedTransactions.filter((transaction) =>
      supportedNetworks.every((value) => value.genesisHash !== encodeBase64(transaction.genesisHash))
    ...
    if (unsupportedTransactionsByNetwork.length > 0) {
      this._logger?.debug(
        `${AVMWebProviderMessageHandler.name}#${_functionName}: transactions [${unsupportedTransactionsByNetwork
      ...
      return await this._sendResponseToMiddleware(
        new AVMWebProviderResponseMessage<ISignTransactionsResult>({
          error: new ARC0027NetworkNotSupportedError({
            genesisHashes: uniqueGenesisHashesFromTransactions(unsupportedTransactionsByNetwork),
            providerId: __PROVIDER_ID__,
          }),
          id: generateUUID(),
          method: message.payload.method,
    ...
    genesisHashes = uniqueGenesisHashesFromTransactions(decodedUnsignedTransactions);
    filteredSessions = await this._fetchSessions(
      (session) =>
        session.host === message.payload.clientInfo.host && genesisHashes.some((value) => value === session.genesisHash)
    );
  ...
  private async _queueProviderEvent<Params extends TRequestParams>(
    ...
    const events = await this._eventQueueRepository.fetchByType<AVMWebProviderRequestEvent<TRequestParams>>(
