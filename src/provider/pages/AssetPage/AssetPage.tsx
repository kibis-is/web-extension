import { Code, Heading, HStack, IconButton, Text, Tooltip, useDisclosure, VStack } from '@chakra-ui/react';
import BigNumber from 'bignumber.js';
import React, { FC, useEffect } from 'react';
import { useTranslation } from 'react-i18next';
import { IoArrowDownOutline, IoArrowUpOutline, IoEyeOffOutline, IoTrashOutline } from 'react-icons/io5';
...
import { ACCOUNTS_ROUTE, PAGE_ITEM_HEIGHT } from '@provider/constants';

// enums
import { AssetTypeEnum } from '@provider/enums';

// features
import { initializeRemoveAssets } from '@provider/features/remove-assets';
import { initialize as initializeSendAssets } from '@provider/features/send-assets';
...
// pages
import SkeletonAssetPage from '@provider/pages/SkeletonAssetPage';
...
const AssetPage: FC = () => {
  ...
  // hooks
  const { account, accountInformation, asset, assetHolding, amountInStandardUnits } = useAssetPage({
  ...
  if (!account || !accountInformation || !asset || !assetHolding || fetchingAssets) {
    return <SkeletonAssetPage />;
  }

  accountAddress = convertPublicKeyToAVMAddress(AccountRepository.decode(account.publicKey));
  ...
  return (
    <>
      ...
      <PageHeader
        colorMode={colorMode}
        subTitle={account.name ? ellipseAddress(accountAddress, { end: 10, start: 10 }) : undefined}
        title={account.name || ellipseAddress(accountAddress, { end: 10, start: 10 })}
      />

      <VStack alignItems="center" justifyContent="flex-start" px={DEFAULT_GAP - 2} spacing={DEFAULT_GAP - 2} w="full">
        <VStack alignItems="center" justifyContent="flex-start" spacing={1} w="full">
          ...
          <HStack alignItems="center" justifyContent="center" spacing={DEFAULT_GAP / 3} w="full">
            {/*amount*/}
            <Tooltip
            ...
            {/*hide arc-0200 asset*/}
            {asset.type === AssetTypeEnum.ARC0200 && (
              <Tooltip label={t<string>('labels.hideAsset', { context: asset.type })}>
                <IconButton
            ...
            {!account.watchAccount && asset.type === AssetTypeEnum.Standard && (
              <Tooltip label={t<string>('labels.removeAsset', { context: asset.type })}>
                <IconButton
          ...
          <VStack spacing={0} w="full">
            ...
            {asset.name && (
              <PageItem fontSize="xs" label={t<string>('labels.name')}>
                <Tooltip aria-label="Asset name" label={asset.name}>
                  <Text color={defaultTextColor} fontSize="xs" maxW={200} noOfLines={1} textAlign="center">
                    {asset.name}
                  </Text>
                </Tooltip>
              </PageItem>
            ...
            <PageItem
              fontSize="xs"
              label={
                asset.type === AssetTypeEnum.Standard ? t<string>('labels.assetId') : t<string>('labels.applicationId')
              }
            >
              <HStack spacing={1}>
            ...
            >
              <VStack spacing={0} w="full">
                ...
                <PageItem fontSize="xs" label={t<string>('labels.totalSupply')}>
                  <Tooltip
                    aria-label="Asset amount with unrestricted decimals"
                    label={formatCurrencyUnit(convertToStandardUnit(new BigNumber(asset.totalSupply), asset.decimals), {
                      decimals: asset.decimals,
                      thousandSeparatedOnly: true,
                    })}
                  >
                    <Text color={subTextColor} fontSize="xs">
                      {formatCurrencyUnit(convertToStandardUnit(new BigNumber(asset.totalSupply), asset.decimals), {
                        decimals: asset.decimals,
                      })}
                    </Text>
                  </Tooltip>
                ...
                {asset.type === AssetTypeEnum.Standard && (
                  <>
                    ...
                    <PageItem fontSize="xs" label={t<string>('labels.defaultFrozen')}>
                      <Text color={subTextColor} fontSize="xs">
                        {asset.defaultFrozen ? t<string>('labels.yes') : t<string>('labels.no')}
                      </Text>
                    </PageItem>
                        ...
                        {asset.clawbackAddress && (
                          <HStack spacing={1} w="full">
                            <PageItem fontSize="xs" label={t<string>('labels.clawbackAccount')}>
                              ...
                              {/*open in explorer button*/}
                              {!isAccountKnown(accounts, asset.clawbackAddress) && blockExplorer && (
                                <OpenTabIconButton
                                  tooltipLabel={t<string>('captions.openOn', {
                                    name: blockExplorer.canonicalName,
                                  })}
                        ...
                        {asset.freezeAddress && (
                          <HStack spacing={1} w="full">
                            <PageItem fontSize="xs" label={t<string>('labels.freezeAccount')}>
                              ...
                              {/*open in explorer button*/}
                              {!isAccountKnown(accounts, asset.freezeAddress) && blockExplorer && (
                                <OpenTabIconButton
                                  tooltipLabel={t<string>('captions.openOn', {
                                    name: blockExplorer.canonicalName,
                                  })}
                        ...
                        {asset.managerAddress && (
                          <HStack spacing={1} w="full">
                            <PageItem fontSize="xs" label={t<string>('labels.managerAccount')}>
                              ...
                              {/*open in explorer button*/}
                              {!isAccountKnown(accounts, asset.managerAddress) && blockExplorer && (
                                <OpenTabIconButton
                                  tooltipLabel={t<string>('captions.openOn', {
                                    name: blockExplorer.canonicalName,
                                  })}
                        ...
                        {asset.reserveAddress && (
                          <HStack spacing={1} w="full">
                            <PageItem fontSize="xs" label={t<string>('labels.reserveAccount')}>
                              ...
                              {/*open in explorer button*/}
                              {!isAccountKnown(accounts, asset.reserveAddress) && blockExplorer && (
                                <OpenTabIconButton
                                  tooltipLabel={t<string>('captions.openOn', {
                                    name: blockExplorer.canonicalName,
                                  })}
