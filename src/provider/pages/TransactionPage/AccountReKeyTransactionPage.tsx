...
// types
import type { IAccountReKeyTransaction } from '@provider/types';
...
const AccountReKeyTransactionPage: FC<IProps<IAccountReKeyTransaction>> = ({ network, transaction }) => {
  ...
  const isReKeyedKnown =
    accounts.findIndex(
      (value) => convertPublicKeyToAVMAddress(PrivateKeyRepository.decode(value.publicKey)) === transaction.rekeyTo
    ) > -1;
  const isSenderKnown =
    accounts.findIndex(
  ...
  return (
    <>
      ...
      <VStack alignItems="flex-start" justifyContent="flex-start" px={DEFAULT_GAP} spacing={DEFAULT_GAP - 2} w="full">
        ...
        {/*re-keyed from*/}
        {transaction.authAddr && transaction.authAddr !== transaction.sender && (
