...
// selectors
import { useSelectARC0200AssetsBySelectedNetwork } from '@provider/selectors';

// types
import type { IARC0200AssetTransferTransaction } from '@provider/types';

// utils
import parseARC0200Transaction from '@provider/utils/parseARC0200Transaction';
...
const TransactionPage: FC = () => {
  ...
  // hooks
  const { account, accounts, network, transaction } = useTransactionPage(transactionId || null);
  // misc
  const reset = () =>
  ...
  switch (transaction.type) {
    ...
    case TransactionTypeEnum.ApplicationUpdate:
      return (
        <ApplicationTransactionPage account={account} accounts={accounts} network={network} transaction={transaction} />
      );
    case TransactionTypeEnum.ApplicationNoOp:
      if (arc0200Assets.find(({ id }) => id === transaction.applicationId)) {
        ...
        if (arc0200Transaction?.type === TransactionTypeEnum.ARC0200AssetTransfer) {
          return (
    ...
    case TransactionTypeEnum.AssetConfig:
      return (
        <AssetConfigTransactionPage account={account} accounts={accounts} network={network} transaction={transaction} />
      );
    case TransactionTypeEnum.AssetCreate:
      return (
        <AssetCreateTransactionPage account={account} accounts={accounts} network={network} transaction={transaction} />
      );
    ...
    case TransactionTypeEnum.AssetUnfreeze:
      return (
        <AssetFreezeTransactionPage account={account} accounts={accounts} network={network} transaction={transaction} />
      );
    ...
    case TransactionTypeEnum.Payment:
      return (
        <PaymentTransactionPage account={account} accounts={accounts} network={network} transaction={transaction} />
      );
    default:
      return null;
  }
