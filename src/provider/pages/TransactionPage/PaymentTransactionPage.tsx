...
const PaymentTransactionPage: FC<IProps<IPaymentTransaction>> = ({ account, accounts, network, transaction }) => {
  ...
  const isReceiverKnown =
    accounts.findIndex((value) => convertPublicKeyToAVMAddress(value.publicKey) === transaction.receiver) > -1;
  const isSenderKnown =
    accounts.findIndex((value) => convertPublicKeyToAVMAddress(value.publicKey) === transaction.sender) > -1;
  // handlers
