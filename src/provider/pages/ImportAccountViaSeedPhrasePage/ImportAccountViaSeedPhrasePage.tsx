...
const ImportAccountViaSeedPhrasePage: FC<IAddAccountPageProps> = ({ onComplete, saving }) => {
  const { t } = useTranslation();
  const dispatch = useDispatch<IAppThunkDispatch<IMainRootState | IRegistrationRootState>>();
  ...
  const [seedPhrases, setSeedPhrases] = useState<string[]>(Array.from({ length: 25 }, () => ''));
  const [seedPhraseError, setSeedPhraseError] = useState<string | null>(null);
  // misc
  const _context = 'import-account-via-seed-phrase-page';
  const stepsLabels = [t<string>('headings.enterYourSeedPhrase'), t<string>('headings.nameYourAccount')];
  ...
  const handleImportClick = async () => {
    ...
    if (!keyPair) {
      logger.debug(`${ImportAccountViaSeedPhrasePage.name}#${_functionName}: unable to import account, account "null"`);
    ...
    logger.debug(
      `${ImportAccountViaSeedPhrasePage.name}#${_functionName}: importing account "${convertPublicKeyToAVMAddress(
        keyPair.publicKey()
      )}"`
    );
  ...
  const handleOnMnemonicPhraseChange = (value: string[]) => setSeedPhrases(value);
  const handleNextClick = () => {
    let _seedPhraseError: string | null;

    // if this is the first step, validate the mnemonic
    if (activeStep <= StepsEnum.SeedPhrase) {
      _seedPhraseError = validate(t<string>('labels.seedPhrase'), seedPhrases, t);
      ...
      <VStack flexGrow={1} justifyContent="center" pb={DEFAULT_GAP} px={DEFAULT_GAP} spacing={DEFAULT_GAP / 3} w="full">
        ...
        >
          ...
          <Step label={stepsLabels[StepsEnum.SeedPhrase]}>
            <VStack alignItems="flex-start" p={1} spacing={DEFAULT_GAP / 3} w="full">
