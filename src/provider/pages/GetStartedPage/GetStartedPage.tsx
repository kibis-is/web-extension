...
import { CREATE_PASSWORD_ROUTE } from '@provider/constants';
...
const GetStartedPage: FC = () => {
  ...
  return (
    <VStack flexGrow={1} pb={DEFAULT_GAP} px={DEFAULT_GAP} spacing={DEFAULT_GAP / 3} w="full">
      <VStack flexGrow={1} justifyContent="center" spacing={DEFAULT_GAP / 3} w="full">
        <KibisisIcon color={primaryColor} boxSize={iconSize} />

        <Heading color={defaultTextColor}>{__APP_TITLE__}</Heading>

        {__VERSION__ && <Text color={subTextColor} fontSize="sm">{`v${__VERSION__}`}</Text>}
      </VStack>
