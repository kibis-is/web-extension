...
export default function useNFTPage({ appId, tokenId }: IUseNFTPageOptions): IUseNFTPageState {
  // selectors
  const account: IAccount | null = useSelectActiveAccount();
  const accountInformation: IAccountInformation | null = useSelectActiveAccountInformation();
  const arc0072Assets: IARC0072Asset[] = useSelectARC0072AssetsBySelectedNetwork();
  // state
  const [asset, setAsset] = useState<IARC0072Asset | null>(null);
  const [assetHolding, setAssetHolding] = useState<IARC0072AssetHolding | null>(null);
  ...
  useEffect(() => {
    ...
    if (accountInformation && appId && tokenId) {
      selectedAssetHolding =
        accountInformation.arc0072AssetHoldings.find((value) => value.id === appId && value.tokenId === tokenId) ||
        null;
