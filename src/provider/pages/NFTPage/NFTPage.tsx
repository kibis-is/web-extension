import { Code, Heading, HStack, Image, Link, Skeleton, Text, Tooltip, VStack } from '@chakra-ui/react';
...
// images
import nftPlaceholderImage from '@provider/images/placeholder_nft.png';
      ...
      <VStack alignItems="center" justifyContent="flex-start" px={DEFAULT_GAP - 2} spacing={DEFAULT_GAP - 2} w="full">
        <VStack alignItems="center" justifyContent="flex-start" spacing={DEFAULT_GAP / 3} w="full">
          ...
          {assetHolding.metadata.name ? (
            <Tooltip aria-label="Name of NFT" label={assetHolding.metadata.name}>
              <Heading color={defaultTextColor} maxW={250} noOfLines={1} size="lg" textAlign="center">
                {assetHolding.metadata.name}
              </Heading>
            </Tooltip>
          ) : (
          ...
          {assetHolding.metadata.description && (
            <Tooltip aria-label="NFT description" label={assetHolding.metadata.description}>
              <Text color={defaultTextColor} fontSize="sm" maxW={250} noOfLines={4} textAlign="center">
                {assetHolding.metadata.description}
              </Text>
            </Tooltip>
          )}
          ...
          <PageItem fontSize="sm" label={t<string>('labels.tokenId')}>
            <Text color={subTextColor} fontSize="sm">
              {asset ? `${assetHolding.tokenId}/${asset.totalSupply}` : assetHolding.tokenId}
            </Text>
          </PageItem>
