...
import { IoKeyOutline, IoLockClosedOutline, IoPushOutline } from 'react-icons/io5';
...
} from '@provider/features/credential-lock';
...
const SecuritySettingsPage: FC = () => {
  ...
  // handlers
  const handleOnEnableCredentialsLockSwitchChange = async (event: ChangeEvent<HTMLInputElement>) => {
  ...
  const handleOnAuthenticationModalConfirm = async (result: TEncryptionCredentials) => {
    ...
    try {
      if (result.type === EncryptionMethodEnum.Unencrypted) {
        throw new MalformedDataError('enabling credential lock requires encryption credentials');
      }
  ...
  return (
    <>
      ...
      <PageHeader colorMode={colorMode} title={t<string>('titles.page', { context: 'security' })} />
      ...
      <VStack w="full">
        ...
        <SettingsSelectItem
          ...
          value={
            durationOptions.find((value) => value.value === settings.security.credentialLockTimeoutDuration) || {
              label: t<string>('labels.credentialsLockDuration', {
                context: CREDENTIALS_LOCK_DURATION_NORMAL,
              }),
              value: CREDENTIALS_LOCK_DURATION_NORMAL,
