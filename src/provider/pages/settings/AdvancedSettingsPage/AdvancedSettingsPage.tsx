...
import { ADVANCED_ROUTE, CUSTOM_NODES_ROUTE, SETTINGS_ROUTE } from '@provider/constants';

// features
import { saveActiveAccountDetails } from '@provider/features/accounts';
import { saveToStorageThunk as saveSettingsToStorageThunk } from '@provider/features/settings';

// selectors
import { useSelectActiveAccountDetails, useSelectSettings, useSelectSettingsColorMode } from '@provider/selectors';

// types
import type { IAdvancedSettings, IAppThunkDispatch, IMainRootState } from '@provider/types';
...
const AdvancedSettingsPage: FC = () => {
  ...
  // handlers
  const handleOnSwitchChange = (key: keyof IAdvancedSettings) => (event: ChangeEvent<HTMLInputElement>) => {
    dispatch(
  ...
  const handleOnAllowAccountPasskeysChange = useCallback(
    (event: ChangeEvent<HTMLInputElement>) => {
      ...
      if (!event.target.checked && activeAccountDetails?.tabIndex === AccountTabEnum.Passkeys) {
        dispatch(
  ...
  return (
    <>
      <PageHeader colorMode={colorMode} title={t<string>('titles.page', { context: 'advanced' })} />
