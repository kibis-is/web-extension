...
// modals
import ManageSessionModal from '@provider/modals/ManageSessionModal';
...
// types
import type { IAppThunkDispatch, IMainRootState, ISession } from '@provider/types';
...
const SessionsSettingsPage: FC = () => {
  ...
  const handleOnSelectSession = (id: string) => setSession(sessions.find((value) => value.id === id) || null);
      ...
      return (
        <VStack flexGrow={1} m={0} p={0} spacing={0} w="full">
          {Array.from({ length: 3 }, (_, index) => (
            <SettingsSessionItemSkeleton key={`${_context}-sessions-item-skeleton-${index}`} />
          ))}
        </VStack>
      );
    }
    ...
    return sessions.length > 0 ? (
      <ScrollableContainer direction="column" flexGrow={1} m={0} p={0} spacing={0} w="full">
        {sessions.reduce((acc, currentValue, index) => {
          const network = networks.find((value) => value.genesisHash === currentValue.genesisHash) || null;
  ...
  return (
    <>
      <ManageSessionModal onClose={handleManageSessionModalClose} session={session} />

      <PageHeader colorMode={colorMode} title={t<string>('titles.page', { context: 'sessions' })} />
