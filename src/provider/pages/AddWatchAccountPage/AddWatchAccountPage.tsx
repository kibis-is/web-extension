...
const AddWatchAccountPage: FC<IProps> = ({ onComplete, saving }) => {
  ...
  // misc
  const stepsLabels = [t<string>('headings.enterAnAddress'), t<string>('headings.nameYourAccount')];
  const hasCompletedAllSteps = activeStep === stepsLabels.length;
  // handlers
  const handleNextClick = () => {
    if (
      (activeStep === StepsEnum.Address && (!!addressError || !!addressValidate(addressValue))) ||
      (activeStep === StepsEnum.AccountName && (!!nameError || !!nameValidate(nameValue)))
    ) {
      return;
    }
  ...
  return (
    <>
      {/*page title*/}
      <PageHeader colorMode={colorMode} title={t<string>('titles.page', { context: 'addWatchAccount' })} />
