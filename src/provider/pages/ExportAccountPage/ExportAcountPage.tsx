import { Box, Icon, Spacer, Text, useDisclosure, VStack } from '@chakra-ui/react';
...
// images
import qrCodePlaceholderImage from '@provider/images/placeholder_qr_code.png';

// modals
import { AccountSelectModal } from '@provider/components/accounts/AccountSelect';
...
import type { IExportAccount } from '@provider/utils/createAccountImportURI';
...
const ExportAccountPage: FC = () => {
  ...
  const [selectedAccounts, setSelectedAccounts] = useState<IAccountWithExtendedProps[] | null>(null);
  ...
  const handleOnAuthenticationModalConfirm = async (result: TEncryptionCredentials) => {
    ...
    if (!selectedAccounts) {
      logger.debug(`${ExportAccountPage.name}#${_functionName}: no accounts selected`);
    ...
    try {
      uris = createAccountImportURI({
        accounts: await Promise.all(
          selectedAccounts.map<Promise<IExportAccount>>(async ({ name, publicKey }) => {
            switch (result.type) {
            ...
            if (!keyPair) {
              throw new DecryptionError(
                `failed to get private key for account "${convertPublicKeyToAVMAddress(publicKey)}"`
              );
            }
      ...
      setPagination([1, Math.ceil(selectedAccounts.length / EXPORT_ACCOUNT_PAGE_LIMIT)]); // set the pagination
    } catch (error) {
  ...
  return (
    <>
      ...
      <PageHeader colorMode={colorMode} title={t<string>('titles.page', { context: 'exportAccount' })} />
            ...
            <>
              ...
              {!svgStrings ? (
                <Box
                  ...
                  w={`${qrCodeSize}px`}
                >
                  <Icon as={IoEyeOffOutline} color="gray.600" h={placeholderIconSize} w={placeholderIconSize} />
                </Box>
              ) : (
                <>
                  <Box
