...
import { ACCOUNT_PAGE_HEADER_ITEM_HEIGHT, ACCOUNTS_ROUTE, ADD_ACCOUNT_ROUTE, PASSKEY_ROUTE } from '@provider/constants';
...
import {
  ...
  updateAccountsThunk,
} from '@provider/features/accounts';
import { openConfirmModal, setWhatsNewModal } from '@provider/features/layout';
import { openModal as openMoveGroupModal } from '@provider/features/move-group-modal';
import { create as createNotification } from '@provider/features/notifications';
import { updateTransactionParamsForSelectedNetworkThunk } from '@provider/features/networks';
import { setAccountAndType as setReKeyAccount } from '@provider/features/re-key-account';
import { saveToStorageThunk as saveSettingsToStorageThunk } from '@provider/features/settings';
import { savePolisAccountIDThunk } from '@provider/features/system';

// hooks
import usePrimaryColorScheme from '@provider/hooks/usePrimaryColorScheme';

// icons
import BsFolderMove from '@provider/icons/BsFolderMove';

// modals
import AVMNameModal from '@provider/modals/avm-names/AVMNameModal';
import EditAccountModal from '@provider/modals/EditAccountModal';
import ShareAddressModal from '@provider/modals/ShareAddressModal';
...
import isReKeyedAuthAccountAvailable from '@provider/utils/isReKeyedAuthAccountAvailable';
import StakingAppModal from '@provider/modals/StakingAppModal';
...
const AccountPage: FC = () => {
  ...
  const updatingActiveAccountTransactions = useSelectActiveAccountTransactionsUpdating();
  // hooks
  const primaryColorScheme = usePrimaryColorScheme();
  // states
  const [selectedName, setSelectedName] = useState<IARC0072AssetHolding | null>(null);
  const [selectedStakingApp, setSelectedStakingApp] = useState<IAccountStakingApp | null>(null);
  // misc
  ...
  const handleOnAVMNameModalClose = useCallback(() => setSelectedName(null), [setSelectedName]);
  const handleOnEditAccountClick = () => onEditAccountModalOpen();
  const handleOnMakePrimaryClick = () => account && dispatch(savePolisAccountIDThunk(account.id));
  const handleOnMoveGroupClick = () => account && dispatch(openMoveGroupModal(account.id));
  ...
  const handleOnRemovePasskeyClick = useCallback(
    (id: string) => {
      const passkey = account?.passkeys.find((value) => value.id === id) || null;
  ...
  const handleOnStakingAppModalClose = useCallback(() => setSelectedStakingApp(null), [setSelectedStakingApp]);
  const handleOnViewAVMNameClick = useCallback(
    (id: string) => setSelectedName(accountInformation?.enVoi.items.find(({ tokenId }) => id === tokenId) || null),
    [accountInformation, setSelectedName]
  );
  const handleOnViewPasskeyClick = useCallback((id: string) => navigate(`${ACCOUNTS_ROUTE}${PASSKEY_ROUTE}/${id}`), []);
  const handleOnViewStakingApp = useCallback(
    (id: string) => setSelectedStakingApp(stakingApps.find(({ appID }) => appID === id) || null),
      ...
      dispatch(
        openConfirmModal({
          description: t<string>('captions.removeAccount', {
            address: ellipseAddress(convertPublicKeyToAVMAddress(PrivateKeyRepository.decode(account.publicKey)), {
              end: 10,
              start: 10,
            }),
          }),
  ...
  const renderContent = () => {
    ...
    if (fetchingAccounts || fetchingSettings) {
      return <AccountPageSkeletonContent {...headerContainerProps} pt={DEFAULT_GAP - 2} />;
    }

    if (account && accountInformation && network) {
      address = convertPublicKeyToAVMAddress(PrivateKeyRepository.decode(account.publicKey));
      ...
      return (
        <>
          ...
          <VStack {...headerContainerProps}>
            {/*top elements*/}
            <HStack minH={ACCOUNT_PAGE_HEADER_ITEM_HEIGHT} pt={DEFAULT_GAP - 2} w="full">
              {/*network connectivity*/}
              {!online && (
                <Tooltip aria-label="Offline icon" label={t<string>('captions.offline')}>
                  <span
                    style={{
                      height: '1em',
                      lineHeight: '1em',
              ...
              {/*network selection*/}
              <NetworkSelect networks={networks} onSelect={handleNetworkSelect} size="xs" value={network} />
            </HStack>
            ...
            <HStack alignItems="center" justifyContent="flex-end" spacing={1} w="full">
              <NativeBalance
                ...
                minAtomicBalance={new BigNumber(accountInformation.minAtomicBalance)}
            ...
            >
              ...
              <OverflowMenu
                items={[
                  // make primary
                  ...(!account || !systemInfo || systemInfo.polisAccountID !== account.id
                    ? [
                        {
                          icon: IoStarOutline,
            ...
            <VStack alignItems="flex-end" spacing={DEFAULT_GAP / 3} w="full">
              <HStack alignItems="center" spacing={DEFAULT_GAP / 3} justifyContent="flex-end" w="full">
                {/*polis account badge*/}
                {account && systemInfo && systemInfo.polisAccountID === account.id && <PolisAccountBadge />}
          ...
          <Tabs
            ...
            defaultIndex={activeAccountDetails?.tabIndex || AccountTabEnum.Assets}
            isLazy={true}
            m={0}
          ...
          >
            ...
            >
              ...
              {settings.advanced.allowAccountPasskeys && (
                <Tab isDisabled={account.watchAccount}>{t<string>('labels.passkeys')}</Tab>
              )}
  ...
  return (
    <>
      {account && (
        <>
          <EditAccountModal isOpen={isEditAccountModalOpen} onClose={onEditAccountModalClose} />
          <AVMNameModal item={selectedName} network={network} onClose={handleOnAVMNameModalClose} />
          <StakingAppModal app={selectedStakingApp} network={network} onClose={handleOnStakingAppModalClose} />
          <ShareAddressModal
            address={convertPublicKeyToAVMAddress(PrivateKeyRepository.decode(account.publicKey))}
            isOpen={isShareAddressModalOpen}
            onClose={onShareAddressModalClose}
      ...
      <VStack alignItems="center" justifyContent="flex-start" flexGrow={1} w="full">
        {renderContent()}
      </VStack>
    </>
  );
