...
// types
import type { IARC0200AssetInformation } from '@provider/types';
import type { IABIResult, INewOptions, ISimulateTransaction } from '../BaseContract';
import type { ITransferOptions } from './types';

// utils
import calculateAppMbrForBox from '@provider/utils/calculateAppMbrForBox';
...
export default class ARC0200Contract extends BaseContract {
  ...
  public async balanceOf(address: string): Promise<BigNumber> {
    ...
    try {
      ...
      this._logger.debug(`${ARC0200Contract.name}#${_functionName}: ${error.message}`);
  ...
  public async buildUnsignedTransferTransactions({
    ...
    try {
      ...
      encodedAmount = (abiMethod.args[1].type as ABIType).encode(BigInt(amountInAtomicUnits.toFixed()));
      ...
      if (!boxReferences) {
        throw new Error(`failed to get box references for sender "${fromAddress}" and recipient "${toAddress}"`);
      }

      // if the balance is zero, we will need to create a payment transaction to fund a box
      if (toBalance.lte(0)) {
        this._logger.debug(`${ARC0200Contract.name}#${_functionName}: no balance detected, adding payment for box`);
      ...
      transactions = [...(paymentTransaction ? [paymentTransaction] : []), appWriteTransaction];
  ...
  public async metadata(): Promise<IARC0200AssetInformation> {
    ...
    const [decimalResult, nameResult, symbolResult, totalSupplyResult] = results;
