...
import type { IAVMAccountInformation } from '@provider/types';
import type {
  IABIResult,
...
export default class BaseContract {
  ...
  constructor({ algod, appId, feeSinkAddress, logger }: INewOptions) {
    ...
    this._logger = logger || createLogger(__ENV__ === 'development' ? 'debug' : 'error');
  }
   ...
   */
  public static formatBase64EncodedUintArg(arg: string, size: number): BigNumber {
    return new BigNumber(String(new ABIUintType(size).decode(decodeBase64(arg))));
  }
  ...
  protected async determineBoxReferences({
    ...
    fromAddress,
    suggestedParams,
  }: IDetermineBoxReferencesOptions): Promise<algosdk.modelsv2.BoxReference[] | null> {
    ...
    try {
      ...
      this._logger.debug(`${BaseContract.name}#${_functionName}: ${error.message}`);

      throw error;
    }
  }

  protected parseTransactionResponse({ abiMethod, response }: IParseTransactionResponseOptions): IABIResult | null {
    ...
    if (type.includes('uint')) {
      return new BigNumber(String(abiMethod.returns.type.decode(trimmedLog) as bigint));
    }
  ...
  protected async readByMethod({
    ...
    try {
      ...
      if (response.txnGroups[0].failureMessage) {
        throw new ReadABIContractError(this._appId, response.txnGroups[0].failureMessage);
      }
  ...
  protected async simulateTransactions(
      ...
      txnGroups: [
        new algosdk.modelsv2.SimulateRequestTransactionGroup({
          txns: transactions.map((value, index) => {
            ...
            return {
              ...(decodeObj(encodeUnsignedSimulateTransaction(value)) as EncodedSignedTransaction),
              ...(authAddress && {
    ...
    return await this._algod.simulateTransactions(request).setIntDecoding(IntDecoding.BIGINT).do();
  }
  ...
  public async applicationAccountInformation(): Promise<IAVMAccountInformation> {
    return (await this._algod.accountInformation(this.applicationAddress()).do()) as IAVMAccountInformation;
  }
  ...
  public async boxByName(name: Uint8Array): Promise<algosdk.modelsv2.Box | null> {
    ...
    try {
      return await this._algod.getApplicationBoxByName(new BigNumber(this._appId).toNumber(), name).do();
    } catch (error) {
      this._logger.debug(`${BaseContract.name}#${_functionName}: ${error.message}`);
