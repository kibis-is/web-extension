...
// utils
import isZeroAddress from '@provider/utils/isZeroAddress';
...
export default class ARC0072Contract extends BaseContract {
  ...
  public async balanceOf(owner: string): Promise<BigNumber> {
    ...
    try {
      ...
      this._logger.debug(`${ARC0072Contract.name}#${_functionName}: ${error.message}`);
  ...
  public async ownerOf(tokenId: BigNumber): Promise<string | null> {
    ...
    try {
      ...
      encodedTokenId = (abiMethod.args[0].type as ABIType).encode(BigInt(tokenId.toFixed()));
      result = (await this.readByMethod({
        abiMethod,
  ...
  public async tokenByIndex(index: BigNumber): Promise<BigNumber> {
    ...
    try {
      ...
      encodedIndex = (abiMethod.args[0].type as ABIType).encode(BigInt(index.toFixed()));
      result = (await this.readByMethod({
        abiMethod,
