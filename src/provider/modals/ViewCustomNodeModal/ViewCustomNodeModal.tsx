import { Heading, Modal, ModalBody, ModalContent, ModalFooter, ModalHeader } from '@chakra-ui/react';
...
// selectors
import { useSelectNetworks, useSelectSettingsColorMode } from '@provider/selectors';
...
const ViewCustomNodeModal: FC<IProps> = ({ item, onClose }) => {
  ...
  useEffect(() => {
    ...
    setNetwork(networks.find((value) => value.genesisHash === item.genesisHash) || null);
  }, [item]);
    ...
    >
      <ModalContent backgroundColor={BODY_BACKGROUND_COLOR} borderTopRadius={theme.radii['3xl']} borderBottomRadius={0}>
        ...
        <ModalBody display="flex" px={DEFAULT_GAP}>
          {item && network && <CustomNodeSummaryModalContent item={item} network={network} />}
        </ModalBody>
