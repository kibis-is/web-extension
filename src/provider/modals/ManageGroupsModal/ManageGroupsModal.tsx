...
import { IoFolderOutline, IoPencilOutline, IoTrashOutline } from 'react-icons/io5';
...
import { ACCOUNT_GROUP_NAME_BYTE_LIMIT } from '@provider/constants';

// features
import { removeGroupByIDThunk, saveAccountGroupsThunk } from '@provider/features/accounts';
...
const ManageGroupsModal: FC<IModalProps> = ({ onClose }) => {
  ...
  const handleOnAddSubmit = async () => {
    ...
    // add the new group
    await dispatch(saveAccountGroupsThunk([AccountGroupRepository.initializeDefaultAccountGroup(nameValue)])).unwrap();
  ...
  const handleOnRemoveClick = (id: string) => async () => {
    ...
    numberOfAccounts = AccountGroupRepository.numberOfAccountsInGroup(id, accounts);
    ...
    return (
      <ScrollableContainer showScrollBars={true} w="full">
        {groups.map((value) => (
          ...
          >
            {/*icon*/}
            <Icon as={IoFolderOutline} boxSize={calculateIconSize('md')} color={defaultTextColor} />

            {/*name*/}
            <HStack flexGrow={1} spacing={1} w="full">
              <Text color={defaultTextColor} textAlign="left">
                {`(${AccountGroupRepository.numberOfAccountsInGroup(value.id, accounts)})`}
              </Text>
    ...
    <Modal isOpen={isOpen} motionPreset="slideInBottom" onClose={handleClose} size="full" scrollBehavior="inside">
      <ModalContent backgroundColor={BODY_BACKGROUND_COLOR} borderTopRadius={theme.radii['3xl']} borderBottomRadius={0}>
        ...
        <ModalFooter p={DEFAULT_GAP}>
          {/*cancel button*/}
          <Button colorMode={colorMode} onClick={handleCancelClick} size="lg" variant="solid" w="full">
