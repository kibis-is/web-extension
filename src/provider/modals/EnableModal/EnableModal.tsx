...
import { sendEnableResponseThunk } from '@provider/features/messages';
import { saveToStorage as saveSessionToStorage } from '@provider/features/sessions';
...
// types
import type { IAppThunkDispatch, IBackgroundRootState, IMainRootState, IModalProps, ISession } from '@provider/types';

// utils
import convertPublicKeyToAVMAddress from '@common/utils/convertPublicKeyToAVMAddress';
import ellipseAddress from '@common/utils/ellipseAddress';
import mapSessionFromEnableRequest from '@provider/utils/mapSessionFromEnableRequest';
...
const EnableModal: FC<IModalProps> = ({ onClose }) => {
  ...
  const { availableAccounts, event, network, setAvailableAccounts, setNetwork } = useEnableModal();
  ...
  const handleOnAccountCheckChange = (address: string) => (event: ChangeEvent<HTMLInputElement>) => {
    if (!event) {
      return;
    }
    ...
    // remove if unchecked
    setAuthorizedAddresses(authorizedAddresses.filter((value) => value !== address));
  };
  const handleOnSelectAllCheckChange = () => {
    if (authorizedAddresses.length <= 0) {
      return setAuthorizedAddresses(
        availableAccounts?.map((value) => convertPublicKeyToAVMAddress(value.publicKey)) || []
      );
    }
  ...
  const renderContent = () => {
    ...
    if (!availableAccounts || !event || fetching || !network) {
      return Array.from({ length: 3 }, (_, index) => (
        <HStack key={`${_context}-fetching-item-${index}`} py={DEFAULT_GAP - 2} spacing={DEFAULT_GAP - 2} w="full">
    ...
    accountNodes = availableAccounts.reduce<ReactNode[]>((acc, account, currentIndex) => {
      const address = convertPublicKeyToAVMAddress(AccountRepository.decode(account.publicKey));
      ...
      return [
        ...
        >
          ...
          {account.name ? (
            ...
            <Text color={defaultTextColor} flexGrow={1} fontSize="md" textAlign="left">
              {ellipseAddress(address, {
                end: 10,
                start: 10,
              })}
  ...
  const renderHeader = () => {
    ...
    return (
      <VStack alignItems="center" spacing={DEFAULT_GAP - 2} w="full">
        <ClientHeader
          description={event.payload.message.payload.clientInfo.description || undefined}
          iconUrl={event.payload.message.payload.clientInfo.iconURL || undefined}
        ...
        {availableAccounts && (
          <Stack alignItems="flex-end" justifyContent="center" w="full">
            <Tooltip aria-label={t<string>('labels.selectAllAccounts')} label={t<string>('labels.selectAllAccounts')}>
              <Checkbox
                colorScheme={primaryColorScheme}
                isChecked={authorizedAddresses.length === availableAccounts.length}
                isIndeterminate={
                  authorizedAddresses.length > 0 && authorizedAddresses.length < availableAccounts.length
                }
  ...
  useEffect(() => {
    if (availableAccounts && availableAccounts.length > 0 && authorizedAddresses.length <= 0) {
      setAuthorizedAddresses([convertPublicKeyToAVMAddress(AccountRepository.decode(availableAccounts[0].publicKey))]);
    }
  }, [availableAccounts]);
