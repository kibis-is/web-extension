...
import React, { ChangeEvent, FC, ReactNode, useMemo, useRef, useState } from 'react';
...
// features
import { addARC0200AssetHoldingsThunk, addStandardAssetHoldingsThunk } from '@provider/features/accounts';
import {
  clearAssets,
  ...
  setConfirming,
  setSelectedAsset,
} from '@provider/features/add-assets';
...
import isNumericString from '@provider/utils/isNumericString';
...
const AddAssetsModal: FC<IModalProps> = ({ onClose }) => {
  ...
  const isOpen = useMemo<boolean>(() => {
    ...
    accountInformation = AccountRepository.extractAccountInformationForNetwork(account, selectedNetwork);
  ...
  const [queryARC0200AssetDispatch, setQueryARC0200AssetDispatch] = useState<IAppThunkDispatchReturn<
    IQueryARC0200AssetPayload,
    IQueryByIdAsyncThunkConfig,
    IAssetsWithNextToken<IARC0200Asset>
  > | null>(null);
  const [queryStandardAssetDispatch, setQueryStandardAssetDispatch] = useState<IAppThunkDispatchReturn<
    IQueryStandardAssetPayload,
    IQueryByIdAsyncThunkConfig,
  ...
  const handleAddARC0200AssetClick = async () => {
    if (!selectedNetwork || !account || !selectedAsset || selectedAsset.type !== AssetTypeEnum.ARC0200) {
      return;
    }
  ...
  const handleOnAuthenticationModalConfirm = async (result: TEncryptionCredentials) => {
    if (!selectedNetwork || !account || !selectedAsset || selectedAsset.type !== AssetTypeEnum.Standard) {
      return;
    }
    ...
    try {
      ...
      dispatch(
        createNotification({
          title: t<string>('headings.addedAsset', {
            symbol: selectedAsset.unitName || selectedAsset.name || selectedAsset.id,
          }),
          type: 'success',
        })
      );
  ...
  const handleOnScrollEnd = () => {
    if (
      ...
      assetContainerRef.current.scrollHeight -
        (assetContainerRef.current.clientHeight + assetContainerRef.current.scrollTop) ===
        0
    ) {
      setQueryStandardAssetDispatch(
        dispatch(
  ...
  const handleSelectAssetClick = (asset: IAssetTypes) => dispatch(setSelectedAsset(asset));
  // renders
  const renderContent = () => {
    if (selectedNetwork && account) {
      if (selectedAsset) {
        if (confirming) {
          return <AddAssetsConfirmingModalContent colorMode={colorMode} asset={selectedAsset} />;
        }
        ...
        <VStack px={DEFAULT_GAP} spacing={DEFAULT_GAP / 2} w="full">
          ...
          <InputGroup size="md">
            ...
            <InputRightElement h={INPUT_HEIGHT}>
              {fetching && <Spinner thickness="1px" speed="0.65s" color={defaultTextColor} size="sm" />}
              ...
              asset.type === AssetTypeEnum.Standard ? (
                <AddAssetsStandardAssetItem
                  added={
                    ...
                    ].standardAssetHoldings.find((value) => value.id === asset.id)
                  }
                  asset={asset}
    ...
    if (selectedAsset) {
      ...
      if (selectedAsset.type === AssetTypeEnum.Standard) {
        return (
          <HStack spacing={DEFAULT_GAP - 2} w="full">
            ...
            <Button colorMode={colorMode} onClick={handleAddStandardAssetClick} size="lg" variant="solid" w="full">
      ...
      return (
        <HStack spacing={DEFAULT_GAP - 2} w="full">
          ...
          <Button colorMode={colorMode} onClick={handleAddARC0200AssetClick} size="lg" variant="solid" w="full">
            {t<string>('buttons.addAsset')}
          </Button>
        </HStack>
      );
