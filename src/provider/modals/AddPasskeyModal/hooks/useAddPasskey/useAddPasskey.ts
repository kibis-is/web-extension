...
// features
import { saveToStorageThunk as savePasskeyToStorageThunk } from '@provider/features/passkeys';
...
export default function useAddPasskey(): IState {
  ...
  // actions
  const addPasskeyAction = async ({ passkey, password }: IAddPasskeyActionOptions): Promise<boolean> => {
    ...
    logger?.debug(`${_hookName}#${_functionName}: successfully enabled passkey`);
