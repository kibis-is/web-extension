...
import { CUSTOM_NODE_BYTE_LIMIT } from '@provider/constants';

// features
import { addCustomNodeThunk } from '@provider/features/networks';
...
import avmVersionsWithDelay from '@provider/utils/avmVersionsWithDelay';
...
const AddCustomNodeModal: FC<IProps> = ({ isOpen, onClose, onComplete }) => {
  ...
  // handlers
  const handleActivateChange = (event: ChangeEvent<HTMLInputElement>) => setActivate(event.target.checked);
  const handleAdvancedToggle = (value: boolean) => (value ? onAdvancedOpen() : onAdvancedClose());
  ...
  const handleNextClick = async () => {
    ...
    logger.debug(
      `${AddCustomNodeModal.name}#${_functionName}: fetching genesis hash for network at "${algodURLValue}${
      ...
      dispatch(
        createNotification({
          description: t<string>('captions.failedToGetCustomNodeDetails', {
            url: `${algodURLValue}${algodPortValue ? `:${algodPortValue}` : ''}`,
          }),
          ephemeral: true,
    ...
    logger.debug(
      ...
      }" for network at "${algodURLValue}${algodPortValue ? `:${algodPortValue}` : ''}"`
    );

    _network = networks.find(({ genesisHash }) => genesisHash === versions.genesis_hash_b64) || null;
  ...
  const renderContent = () => {
    ...
    if (customNode && network) {
      return <CustomNodeSummaryModalContent item={customNode} network={network} />;
    }
