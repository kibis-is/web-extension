...
// features
import { addToGroupThunk, saveAccountGroupsThunk } from '@provider/features/accounts';
...
const MoveGroupModal: FC<IModalProps> = ({ onClose }) => {
      ...
      <ScrollableContainer showScrollBars={true} w="full">
        {groups.map((value) => (
          <ActionItem
            ...
            key={value.id}
            label={`${value.name} (${AccountGroupRepository.numberOfAccountsInGroup(value.id, accounts)})`}
  ...
  return (
    <Modal isOpen={!!account} motionPreset="slideInBottom" onClose={handleClose} size="full" scrollBehavior="inside">
