...
// selectors
import { useSelectLogger, useSelectSettings, useSelectSettingsColorMode } from '@provider/selectors';
...
const ShareAddressModal: FC<IProps> = ({ address, isOpen, onClose }) => {
  ...
  useEffect(() => {
    (async () => {
      try {
        const svg: string = await toString(getFormatFromIndex(pillIndex, address), {
          type: 'svg',
          width: qrCodeSize,
        });
        setSvgString(svg);
  ...
  return (
    <Modal isOpen={isOpen} motionPreset="slideInBottom" onClose={handleClose} size="full">
      <ModalContent backgroundColor={BODY_BACKGROUND_COLOR} borderTopRadius={theme.radii['3xl']} borderBottomRadius={0}>
        ...
        <ModalBody px={DEFAULT_GAP}>
          <VStack alignItems="center" spacing={DEFAULT_GAP / 3} w="full">
            ...
            <HStack alignItems="center" justifyContent="center" spacing={1} w="full">
