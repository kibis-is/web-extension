...
import { IoArrowBackOutline, IoArrowForwardOutline, IoArrowUpOutline } from 'react-icons/io5';
...
import { TRANSACTION_NOTE_BYTE_LIMIT } from '@provider/constants';
...
} from '@provider/features/send-assets';

// hooks
import useAddressInput from '@provider/hooks/useAddressInput';
import useAmountInput from '@provider/hooks/useAmountInput';
import useDefaultTextColor from '@provider/hooks/useDefaultTextColor';
import useGenericInput from '@provider/hooks/useGenericInput';
...
// utils
import calculateMaxTransactionAmount from '@provider/utils/calculateMaxTransactionAmount';
...
const SendAssetModal: FC<IModalProps> = ({ onClose }) => {
  ...
  // state
  const [maximumAmountInAtomicUnits, setMaximumAmountInAtomicUnits] = useState<string>('0');
  const [transactions, setTransactions] = useState<Transaction[] | null>(null);
  // misc
  const allAssets: (IAssetTypes | INativeCurrency)[] = [...arc200Assets, ...standardAssets]
    .sort((a, b) => {
  ...
  // handlers
  const handleOnAssetSelect = (value: IAssetTypes | INativeCurrency) => dispatch(setAsset(value));
  const handleOnSenderAccountSelect = (value: IAccountWithExtendedProps) => dispatch(setSender(value));
  ...
  const handleNextClick = async () => {
    ...
    if (
      ...
      [validateNote(noteValue), validateReceiverAddress(receiverAddressValue)].some((value) => !!value)
    ) {
      return;
    }

    logger.debug(`${SendAssetModal.name}#${_functionName}: creating unsigned transactions`);
    ...
    try {
      ...
      logger.debug(
        `${SendAssetModal.name}#${_functionName}: created unsigned transactions "[${_transactions
  ...
  const handleOnAuthenticationModalConfirm = async (result: TEncryptionCredentials) => {
    ...
    try {
      ...
      logger.debug(
        `${SendAssetModal.name}#${_functionName}: sent transactions [${transactionIds
          .map((value) => `"${value}"`)
          .join(',')}] to the network`
      );

      receiverAccount =
        accounts.find((value) => convertPublicKeyToAVMAddress(value.publicKey) === receiverAddressValue) || null;
      ...
      dispatch(
        updateAccountsThunk({
          accountIDs: receiverAccount ? [sender.id, receiverAccount.id] : [sender.id],
  ...
  const renderContent = () => {
    ...
    if (confirming) {
      return <SendAssetModalConfirmingContent colorMode={colorMode} numberOfTransactions={transactions?.length} />;
    }
