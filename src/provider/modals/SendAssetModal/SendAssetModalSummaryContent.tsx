...
const SendAssetModalSummaryContent: FC<ISendAssetModalSummaryContentProps> = ({
  ...
  // misc
  const amountInAtomicUnits = convertToAtomicUnit(new BigNumber(amountInStandardUnits), asset.decimals);
  ...
  const totalFee = new BigNumber(transactions.reduce((acc, value) => acc + value.fee, 0));
  // renders
  ...
  const renderExtraPaymentItem = () => {
    ...
    extraPaymentInAtomicUnits = new BigNumber(
      String(
        transactions.reduce((acc, value) => (value.type === 'pay' ? (value.amount as bigint) + acc : acc), BigInt(0))
      )
    );
  ...
  return (
    <VStack alignItems="flex-start" justifyContent="flex-start" spacing={DEFAULT_GAP / 3} w="full">
      ...
      <ModalItem
        ...
        value={<AddressDisplay accounts={accounts} address={toAddress} size="sm" network={network} />}
      />
      ...
      {/*note*/}
      {note && note.length > 0 && <ModalTextItem isCode={true} label={`${t<string>('labels.note')}:`} value={note} />}
    </VStack>
  );
};
