...
// selectors
import { useSelectPasskeysSaving, useSelectSettingsColorMode } from '@provider/selectors';
...
const RemovePasskeyModal: FC<IProps> = ({ onClose, removePasskey }) => {
  ...
  const renderContent = () => {
    ...
    if (encrypting) {
      return (
        <VStack alignItems="center" flexGrow={1} justifyContent="center" spacing={DEFAULT_GAP} w="full">
          {/*loader*/}
          <ReEncryptKeysLoadingContent colorMode={colorMode} encryptionProgressState={encryptionProgressState} />
        </VStack>
      );
    }
    ...
    return (
      <VStack alignItems="center" flexGrow={1} justifyContent="flex-start" spacing={DEFAULT_GAP} w="full">
        {/*icon*/}
        <Icon as={GoShieldSlash} color="red.600" h={iconSize} w={iconSize} />

        {/*description*/}
        <Text as="b" color={subTextColor} fontSize="sm" textAlign="center" w="full">
