...
// features
import { removeFromStorageThunk as removePasskeyToStorageThunk } from '@provider/features/passkeys';
...
export default function useRemovePasskey(): IState {
  ...
  // actions
  const removePasskeyAction = async ({ passkey, password }: IRemovePasskeyActionOptions): Promise<boolean> => {
    ...
    logger.debug(`${_hookName}#${_functionName}: requesting input key material from passkey "${passkey.id}"`);
    ...
    logger?.debug(`${_hookName}#${_functionName}: successfully removed passkey`);
