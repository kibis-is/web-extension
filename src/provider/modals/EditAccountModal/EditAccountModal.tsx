...
// features
import { saveAccountDetailsThunk } from '@provider/features/accounts';
...
// selectors
import { useSelectActiveAccount, useSelectAccountsSaving, useSelectSettingsColorMode } from '@provider/selectors';
...
import type { IAccountWithExtendedProps, IAppThunkDispatch, IMainRootState } from '@provider/types';
...
const EditAccountModal: FC<IProps> = ({ isOpen, onClose }) => {
  ...
  // states
  const [color, setColor] = useState<TAccountColors | null>(account?.color || null);
  ...
  const handleSaveClick = async () => {
    ...
    if (!account || !!nameError || [validateName(nameValue)].some((value) => !!value)) {
      return;
    }

    if (account.color === color && account.icon === icon && account && account.name === nameValue) {
      handleClose();
