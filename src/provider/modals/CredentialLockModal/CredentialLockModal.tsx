...
// features
import { deactivateThunk as deactivateCredentialLockThunk } from '@provider/features/credential-lock';
...
const CredentialLockModal: FC = () => {
  ...
  const isOpen =
    !saving && active && settings.security.enableCredentialLock && settings.security.credentialLockTimeoutDuration > 0;
  // handlers
  ...
  const handleOnAuthenticationModalConfirm = async (result: TEncryptionCredentials) => {
    ...
    try {
      if (result.type === EncryptionMethodEnum.Unencrypted) {
        throw new MalformedDataError('deactivating credential lock requires encryption credentials');
      }
      ...
      <Modal isOpen={isOpen} motionPreset="slideInBottom" onClose={handleClose} size="full" scrollBehavior="inside">
        ...
        >
          <ModalBody display="flex" p={DEFAULT_GAP}>
            <VStack flexGrow={1} justifyContent="center" spacing={DEFAULT_GAP} w="full">
              {/*icon*/}
              <KibisisIcon color={primaryColor} h={iconSize} w={iconSize} />

              <VStack justifyContent="center" spacing={DEFAULT_GAP / 3} w="full">
                {/*heading*/}
