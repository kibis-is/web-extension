import { ARC0027MethodCanceledError, ARC0027MethodEnum } from '@agoralabs-sh/avm-web-provider';
import {
  Code,
  HStack,
  Modal,
...
import { sendSignMessageResponseThunk } from '@provider/features/messages';
...
import signBytes from '@provider/utils/signBytes';
...
const SignMessageModal: FC<IModalProps> = ({ onClose }) => {
  ...
  // hooks
  const { authorizedAccounts, event, signer, setAuthorizedAccounts, setSigner } = useSignMessageModal();
  const subTextColor = useSubTextColor();
  // handlers
  const handleAccountSelect = (account: IAccountWithExtendedProps) => setSigner(account);
  ...
  const handleOnAuthenticationModalConfirm = async (result: TEncryptionCredentials) => {
    ...
    logger.debug(`${SignMessageModal.name}#${_functionName}: signing message for signer "${signerAddress}"`);

    try {
      signature = await signBytes({
        bytes: new TextEncoder().encode(event.payload.message.payload.params.message),
        logger,
        publicKey: AccountRepository.decode(signer.publicKey),
        ...result,
      });

      logger.debug(`${SignMessageModal.name}#${_functionName}: signed message for signer "${signerAddress}"`);
  ...
  // renders
  const renderContent = () => {
    if (fetching || !event?.payload.message.payload.params || !authorizedAccounts || !network || !signer) {
      ...
      <VStack spacing={DEFAULT_GAP - 2} w="full">
        ...
        <VStack spacing={DEFAULT_GAP / 3} w="full">
          {event.payload.message.payload.params.signer ? (
            <>
              <Text textAlign="left" w="full">{`${t<string>('labels.addressToSign')}:`}</Text>

              <AccountItem account={signer} colorMode={colorMode} network={network} />
            </>
          ) : (
            <>
              <Text textAlign="left" w="full">{`${t<string>('labels.authorizedAddresses')}:`}</Text>
        ...
        {/*message*/}
        <VStack spacing={DEFAULT_GAP / 3} w="full">
          <Text textAlign="left" w="full">{`${t<string>('labels.message')}:`}</Text>
