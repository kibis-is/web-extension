...
// types
import type { IAccount, IDecodedJwt } from '@provider/types';
...
const SignBytesJwtContent: FC<IProps> = ({ decodedJwt, host, signer }: IProps) => {
  ...
  return (
    <VStack spacing={2} w="full">
      <Text textAlign="left" w="full">{`${t<string>('labels.information')}:`}</Text>
      {/*address/sub*/}
      {decodedJwt.payload.subject && (
        <HStack spacing={2} w="full">
          ...
          {/*value*/}
          <Box backgroundColor={textBackgroundColor} borderRadius={theme.radii['3xl']} px={2} py={1}>
          ...
          {/*warning*/}
          {(!signer || decodedJwt.payload.subject !== convertPublicKeyToAVMAddress(signer.publicKey)) && (
            <WarningIcon tooltipLabel={t<string>('captions.addressDoesNotMatch')} />
          )}
        </HStack>
      )}
      {/*audience/aud*/}
      {decodedJwt.payload.audience && (
        <HStack spacing={2} w="full">
          ...
          {decodedJwt.payload.audience !== host && (
            <WarningIcon tooltipLabel={t<string>('captions.audienceDoesNotMatch')} />
          )}
        </HStack>
      )}
      ...
      <HStack spacing={2} w="full">
        ...
        {decodedJwt.payload.expiresAt < new Date() && (
          <WarningIcon tooltipLabel={t<string>('captions.securityTokenExpired')} />
        )}
      </HStack>
      {/* More information */}
      <Accordion allowToggle={true} w="full">
        <AccordionItem border="none" w="full">
          <AccordionButton px={0}>
            <Text textAlign="left" w="full">{`${t<string>('labels.moreInformation')}:`}</Text>
            <AccordionIcon />
          </AccordionButton>
          <AccordionPanel px={0}>
            <VStack spacing={2} w="full">
              ...
              {decodedJwt.payload.issuedAt && (
                <HStack spacing={2} w="full">
                  <Text fontSize="xs">{`${t<string>('labels.issueDate')}:`}</Text>
                  <Box backgroundColor={textBackgroundColor} borderRadius={theme.radii['3xl']} px={2} py={1}>
                    <Text color={defaultTextColor} fontSize="xs" wordBreak="break-word">
              ...
              {decodedJwt.header.algorithm && (
                <HStack spacing={2} w="full">
                  <Text fontSize="xs">{`${t<string>('labels.signingMethod')}:`}</Text>
                  <Box backgroundColor={textBackgroundColor} borderRadius={theme.radii['3xl']} px={2} py={1}>
                    <Text color={defaultTextColor} fontSize="xs" wordBreak="break-word">
                      {decodedJwt.header.algorithm}
                    </Text>
                  ...
                  {decodedJwt.header.curve && (
                    <Box
                      ...
                      py={1}
                    >
                      <Text color="blue.500" fontSize="xs" wordBreak="break-word">
                        {decodedJwt.header.curve}
                      </Text>
                    </Box>
                  )}
                  ...
                  {decodedJwt.header.algorithm !== 'EdDSA' && (
                    <WarningIcon tooltipLabel={t<string>('captions.invalidAlgorithm')} />
                  )}
                </HStack>
              )}
            </VStack>
