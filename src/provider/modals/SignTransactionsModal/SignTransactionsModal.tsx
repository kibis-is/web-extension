import { ARC0027ErrorCodeEnum, ARC0027MethodCanceledError, ARC0027MethodEnum } from '@agoralabs-sh/avm-web-provider';
import {
  HStack,
  Modal,
  ModalBody,
...
import groupTransactions from '@provider/utils/groupTransactions';
...
const SignTransactionsModal: FC<IModalProps> = ({ onClose }) => {
  ...
  // states
  const [moreDetailsTransactions, setMoreDetailsTransactions] = useState<Transaction[] | null>(null);
  ...
  const renderContent = () => {
    ...
    decodedTransactions = event.payload.message.payload.params.txns.map((value) =>
    ...
    if (groupsOfTransactions.length > 1) {
      return (
        ...
        >
          <GroupOfTransactionsContent groupsOfTransactions={groupsOfTransactions} />
        </MultipleTransactionsContext.Provider>
      );
    }
    ...
    if (groupsOfTransactions[0].length > 1) {
      return <AtomicTransactionsContent transactions={groupsOfTransactions[0]} />;
    }

    // otherwise it is a single transaction
    return <SingleTransactionContent transaction={groupsOfTransactions[0][0]} />;
  };
