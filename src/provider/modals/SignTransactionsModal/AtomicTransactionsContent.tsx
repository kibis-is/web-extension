...
// types
import type { IAccountInformation, IAccountWithExtendedProps, IStandardAsset } from '@provider/types';
...
const AtomicTransactionsContent: FC<IAtomicTransactionsContentProps> = ({ transactions }) => {
  const { t } = useTranslation();
  const genesisHash = uniqueGenesisHashesFromTransactions(transactions).pop() || '';
  ...
  // state
  const [fetchingAccountInformation, setFetchingAccountInformation] = useState<boolean>(false);
  ...
  // misc
  const blockExplorer =
    network?.blockExplorers.find((value) => value.id === preferredExplorer?.id) || network?.blockExplorers[0] || null; // get the preferred explorer, if it exists in the network, otherwise get the default one
  ...
  const handleToggleAccordion = (accordionIndex: number) => (open: boolean) => {
    setOpenAccordions(openAccordions.map((value, index) => (index === accordionIndex ? open : value)));
  };
  // renders
  const renderTransaction = (transaction: Transaction, transactionIndex: number) => {
    ...
    standardAsset = standardAssets.find((value) => value.id === String(transaction.assetIndex)) || null;
      ...
      updatedFromAccounts = await Promise.all(
        transactions.map(async (transaction, index) => {
          ...
          let account: IAccountWithExtendedProps | null =
            accounts.find((value) => value.publicKey.toUpperCase() === encodedPublicKey.toUpperCase()) || null;
          ...
          accountInformation = await updateAccountInformation({
            address: convertPublicKeyToAVMAddress(AccountRepository.decode(encodedPublicKey)),
  ...
  return (
    <VStack spacing={DEFAULT_GAP - 2} w="full">
      {/*group id*/}
      <ModalTextItem label={`${t<string>('labels.groupId')}:`} value={computedGroupId} />
      ...
      {transactions.map((transaction, index) => (
        <Box
          ...
          w="full"
        >
          <VStack alignItems="center" justifyContent="flex-start" spacing={DEFAULT_GAP / 3} w="full">
