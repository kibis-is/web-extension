...
const PaymentTransactionContent: FC<ITransactionBodyProps> = ({
  ...
  const accountInformation = fromAccount
    ...
    : null;
  const amountAsAtomicUnit = new BigNumber(transaction.amount ? String(transaction.amount) : '0');
  const amountInStandardUnits = convertToStandardUnit(amountAsAtomicUnit, network.nativeCurrency.decimals);
  ...
  const renderExtraInformation = () => (
    <>
      ...
      {/*balance*/}
      <ModalAssetItem
        amountInAtomicUnits={new BigNumber(accountInformation?.atomicBalance || '0')}
        decimals={network.nativeCurrency.decimals}
        icon={icon}
        isLoading={loading}
      ...
      {condensed ? (
        ...
        <>
          ...
          <Tooltip
            ...
            label={`${amountInStandardUnits.toString()} ${network.nativeCurrency.symbol}`}
          >
