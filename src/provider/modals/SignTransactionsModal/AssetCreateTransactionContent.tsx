...
const AssetCreateTransactionContent: FC<ITransactionBodyProps> = ({
  ...
  const renderExtraInformation = () => {
    return (
      <>
        ...
        {/*decimals*/}
        <ModalTextItem label={`${t<string>('labels.decimals')}:`} value={String(transaction.assetDecimals)} />

        {/*unit name*/}
        {transaction.assetUnitName && (
          <ModalTextItem label={`${t<string>('labels.unitName')}:`} value={transaction.assetUnitName} />
        )}
        ...
        {transaction.assetURL && (
          <HStack spacing={0} w="full">
            <ModalTextItem isCode={true} label={`${t<string>('labels.url')}:`} value={transaction.assetURL} />

            <OpenTabIconButton size="xs" tooltipLabel={t<string>('captions.openUrl')} url={transaction.assetURL} />
          </HStack>
        )}
  ...
  return (
    <VStack alignItems="flex-start" justifyContent="flex-start" spacing={DEFAULT_GAP / 3} w="full">
      ...
      {/*asset name*/}
      {transaction.assetName && <ModalTextItem label={`${t<string>('labels.name')}:`} value={transaction.assetName} />}
