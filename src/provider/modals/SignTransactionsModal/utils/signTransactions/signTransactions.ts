...
export default async function signTransactions({
  ...
  return await Promise.all(
    arc0001Transactions.map(async (arc001Transaction) => {
      const unsignedTransaction: Transaction = decodeUnsignedTransaction(decodeBase64(arc001Transaction.txn));
      ...
      try {
        signerAddress = convertPublicKeyToAVMAddress(unsignedTransaction.from.publicKey);
      } catch (error) {
      ...
      if (!accounts.some((value) => value.publicKey === AccountRepository.encode(unsignedTransaction.from.publicKey))) {
