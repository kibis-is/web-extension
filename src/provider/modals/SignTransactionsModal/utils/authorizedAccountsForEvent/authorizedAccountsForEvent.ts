...
// types
import type { IAccountInformation, IAccountWithExtendedProps, INetwork } from '@provider/types';
import type { IOptions } from './types';

// utils
import convertPublicKeyToAVMAddress from '@common/utils/convertPublicKeyToAVMAddress';
import decodeUnsignedTransaction from '@provider/utils/decodeUnsignedTransaction';
import getAuthorizedAddressesForHost from '@provider/utils/getAuthorizedAddressesForHost';
...
export default async function authorizedAccountsForEvent({
  ...
  return decodedUnsignedTransactions.reduce<IAccountWithExtendedProps[]>((acc, currentValue) => {
    account =
      accounts.find((value) => value.publicKey === AccountRepository.encode(currentValue.from.publicKey)) || null;
    ...
    authorizedAddresses = getAuthorizedAddressesForHost(
      ...
      sessions.filter((value) => value.genesisHash === base64EncodedGenesisHash)
    );
    network = networks.find((value) => value.genesisHash === base64EncodedGenesisHash) || null;
    accountInformation =
      account && network ? AccountRepository.extractAccountInformationForNetwork(account, network) : null;
    ...
    if (
      ...
      !authorizedAddresses.find((value) => value === convertPublicKeyToAVMAddress(account?.publicKey || '')) ||
