import { ARC0027MethodEnum, ARC0027UnknownError, ISignTransactionsParams } from '@agoralabs-sh/avm-web-provider';
...
import { sendSignTransactionsResponseThunk } from '@provider/features/messages';
import { updateStandardAssetInformationThunk } from '@provider/features/standard-assets';

// selectors
import { useSelectEvents, useSelectLogger, useSelectNetworks, useSelectStandardAssets } from '@provider/selectors';

// types
import type { IAppThunkDispatch, IBackgroundRootState, IMainRootState, INetwork } from '@provider/types';
...
export default function useSignTransactionsModal(): IState {
  ...
  // state
  const [event, setEvent] = useState<AVMWebProviderRequestEvent<ISignTransactionsParams> | null>(null);
    ...
    setEvent(
      (events.find(
        (value) =>
          ...
          value.payload.message.payload.method === ARC0027MethodEnum.SignTransactions
  ...
  useEffect(() => {
    (async () => {
      ...
      if (event && event.payload.message.payload.params && standardAssets && networks.length > 0) {
        try {
          decodedUnsignedTransactions = event.payload.message.payload.params.txns.map((value) =>
        ...
        uniqueGenesisHashesFromTransactions(decodedUnsignedTransactions).forEach((genesisHash) => {
          const network: INetwork | null = networks.find((value) => value.genesisHash === genesisHash) || null;
          const encodedGenesisHash: string = convertGenesisHashToHex(genesisHash).toUpperCase();
          const unknownAssetIds: string[] = decodedUnsignedTransactions
            .filter((value) => value.type === TransactionType.axfer)
            .filter(
              (transaction) =>
                !standardAssets[encodedGenesisHash].some((value) => value.id === String(transaction.assetIndex))
            )
            .map((value) => String(value.assetIndex));

          if (!network) {
            logger.debug(`${_hookName}#useEffect: unable to get network for genesis hash "${genesisHash}"`);
