...
import usePrimaryColorSchemer from '@provider/hooks/usePrimaryColorScheme';

// selectors
import { useSelectNetworks } from '@provider/selectors';
...
const GroupOfTransactionsContent: FC<IGroupOfTransactionsContentProps> = ({ groupsOfTransactions }) => {
  ...
  const handleMoreDetailsClick = (index: number) => () =>
    context && context.setMoreDetailsTransactions(groupsOfTransactions[index] || null);

  // if we have transactions, it means the "more details" button has been pressed
  if (context && context.moreDetailsTransactions && context.moreDetailsTransactions.length > 0) {
    // if the transactions is a greater that one, it will be atomic transactions
    if (context.moreDetailsTransactions.length > 1) {
      return <AtomicTransactionsContent transactions={context.moreDetailsTransactions} />;
    }

    // otherwise it will be a single transaction
    return <SingleTransactionContent transaction={context.moreDetailsTransactions[0]} />;
  }
  ...
  return (
    <VStack spacing={DEFAULT_GAP - 2} w="full">
      {groupsOfTransactions.map((transactions, index) => {
        ...
        const network = networks.find((value) => value.genesisHash === genesisHash) || null;
        ...
        return (
          ...
          >
            <VStack alignItems="center" justifyContent="flex-start" spacing={DEFAULT_GAP / 3} w="full">
              ...
              {/*group id*/}
              {computedGroupId && <ModalTextItem label={`${t<string>('labels.groupId')}:`} value={computedGroupId} />}
