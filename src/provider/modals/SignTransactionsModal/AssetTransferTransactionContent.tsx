...
const AssetTransferTransactionContent: FC<IAssetTransactionBodyProps> = ({
  ...
  // states
  const [amountAsStandardUnit, setAmountAsStandardUnit] = useState<BigNumber>(new BigNumber('0'));
  const [balanceInAtomicUnits, setBalanceInAtomicUnits] = useState<BigNumber | null>();
  // misc
  const accountInformation = fromAccount
    ? AccountRepository.extractAccountInformationForNetwork(fromAccount, network)
    : null;
  const amountInAtomicUnits = new BigNumber(transaction.amount ? String(transaction.amount) : '0');
  const feeInAtomicUnits = new BigNumber(transaction.fee ? String(transaction.fee) : '0');
  // renders
  ...
  useEffect(() => {
    ...
    if (asset) {
      setAmountAsStandardUnit(convertToStandardUnit(amountInAtomicUnits, asset.decimals));

      _balanceInAtomicUnits =
        accountInformation?.standardAssetHoldings.find((value) => value.id === asset.id)?.amount || null;

      setBalanceInAtomicUnits(_balanceInAtomicUnits ? new BigNumber(_balanceInAtomicUnits) : null);
    }
  }, [asset]);
        ...
        <>
          ...
          <Text color={defaultTextColor} fontSize="md" textAlign="left" w="full">
            {t<string>('headings.transaction', {
              context: parseTransactionType(transaction.get_obj_for_encoding(), {
                network,
                sender: fromAccount,
              }),
            })}
