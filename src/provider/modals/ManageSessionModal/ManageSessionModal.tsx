...
} from '@provider/features/sessions';
...
// types
import type { IAppThunkDispatch, IMainRootState, INetwork } from '@provider/types';
...
const ManageSessionModal: FC<IProps> = ({ onClose, session }) => {
  ...
  const handleOnSelectAllCheckChange = () => {
    if (authorizedAddresses.length <= 0) {
      return setAuthorizedAddresses(accounts.map((value) => convertPublicKeyToAVMAddress(value.publicKey)));
    }
        ...
        >
          ...
          {account.name ? (
            <VStack alignItems="flex-start" flexGrow={1} justifyContent="space-evenly" spacing={0}>
              <Text color={defaultTextColor} fontSize="md" maxW={400} noOfLines={1} textAlign="left">
                {account.name}
              </Text>
        ...
        <Stack alignItems="flex-end" justifyContent="center" w="full">
          <Tooltip aria-label={t<string>('labels.selectAllAccounts')} label={t<string>('labels.selectAllAccounts')}>
            <Checkbox
              ...
              isIndeterminate={authorizedAddresses.length > 0 && authorizedAddresses.length < accounts.length}
  ...
  useEffect(() => {
    if (session) {
      ...
      setNetwork(networks.find((value) => value.genesisHash === session.genesisHash) || null);
    }
  }, [session]);

  return (
    <Modal isOpen={!!session} motionPreset="slideInBottom" onClose={handleClose} size="full" scrollBehavior="inside">
