import { Avatar, Button as ChakraButton, HStack, Icon, Text, Tooltip, VStack } from '@chakra-ui/react';
...
const Item: FC<IItemProps> = ({ disabled = false, onClick, passkey }) => {
  ...
  // handlers
  const handleOnClick = useCallback(() => onClick(passkey.id), [passkey, onClick]);
  // renders
  const renderContent = () => (
    <HStack align="center" h="full" justify="space-between" spacing={DEFAULT_GAP / 3} w="full">
      <Avatar
        size="sm"
        {...(passkey.iconURL
          ? {
      ...
      <VStack alignItems="flex-start" flexGrow={1} h="full" justifyContent="space-evenly" spacing={1}>
  ...
  return (
    <ChakraButton
      ...
      px={DEFAULT_GAP / 2}
      py={DEFAULT_GAP / 3}
      rightIcon={<Icon as={IoChevronForward} boxSize={iconSize} color={subTextColor} />}
      variant="ghost"
      w="full"
      {...(!disabled && {
        _hover: {
    ...
    >
      {!disabled ? (
        ...
        <Tooltip label={t<string>('captions.passkeyNotRequested')}>{renderContent()}</Tooltip>
      )}
    </ChakraButton>
  );
};
