...
// errors
import { BaseExtensionError, WebAuthnAuthenticationCanceledError } from '@common/errors';
...
import { sendWebAuthnAuthenticateResponseThunk, sendWebAuthnErrorResponseThunk } from '@provider/features/webauthn';
...
const WebAuthnAuthenticateModal: FC<IModalProps> = ({ onClose }) => {
  ...
  const [event, setEvent] = useState<WebAuthnAuthenticateRequestEvent | null>(null);
  const [selectedPasskeyID, setSelectedPasskeyID] = useState<string | null>(null);
  // memos
  const allowedCredentialIDs = useMemo(
    () => event?.payload.message.payload.options.allowCredentials?.map(({ id }) => encodeUUID(decodeBase64(id))) || [],
    [event]
  );
  const passkeys = useMemo<IAccountPasskey[]>(() => {
    ...
    return account.passkeys.filter(({ rp }) => rp.id === event.payload.message.payload.options.rpId);
    ...
    if (event) {
      await dispatch(
        sendWebAuthnErrorResponseThunk({
          error: new WebAuthnAuthenticationCanceledError(`user dismissed webauthn request modal`),
          event,
        })
      ).unwrap();
      // remove the event
      ...
      <>
        ...
        <VStack align="center" flexGrow={1} spacing={DEFAULT_GAP / 3} w="full">
          <HStack align="center" spacing={DEFAULT_GAP / 2} w="full">
            ...
            <VStack align="start" flexGrow={1} justify="space-evenly" spacing={DEFAULT_GAP / 3} w="full">
              {/*name*/}
          ...
          {passkeys.length > 0 ? (
            <ScrollableContainer>
              {passkeys.map((passkey, index) => (
                <Item
                  disabled={
                    allowedCredentialIDs.length > 0 && !allowedCredentialIDs.some((value) => value === passkey.id)
                  }
  ...
  return (
    <>
      ...
      <AuthenticationModal
        ...
        passwordHint={t<string>('captions.mustEnterPasswordToAuthenticatePasskey')}
      />
