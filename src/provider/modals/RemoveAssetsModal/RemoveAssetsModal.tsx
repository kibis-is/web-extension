...
import { ACCOUNTS_ROUTE } from '@provider/constants';
...
// features
import { removeARC0200AssetHoldingsThunk, removeStandardAssetHoldingsThunk } from '@provider/features/accounts';
import { create as createNotification } from '@provider/features/notifications';
import { setConfirming } from '@provider/features/remove-assets';
...
const RemoveAssetsModal: FC<IRemoveAssetsModalProps> = ({ onClose }) => {
  ...
  const handleRemoveStandardAssetClick = async () => onAuthenticationModalOpen();
  ...
  const renderContent = () => {
    ...
    if (confirming) {
      return <RemoveAssetsConfirmingModalContent colorMode={colorMode} asset={selectedAsset} />;
    }
    ...
    return (
      <VStack alignItems="flex-start" justifyContent="flex-start" px={DEFAULT_GAP} spacing={DEFAULT_GAP - 2} w="full">
        ...
        <VStack spacing={0} w="full">
          ...
          {/*name*/}
          {selectedAsset.name && <ModalTextItem label={`${t<string>('labels.name')}:`} value={selectedAsset.name} />}

          {/*symbol*/}
          {symbol && <ModalTextItem label={`${t<string>('labels.symbol')}:`} value={symbol} />}

          {/*type*/}
          <ModalItem label={`${t<string>('labels.type')}:`} value={<AssetBadge type={selectedAsset.type} />} />
          ...
          {selectedAsset.type === AssetTypeEnum.Standard && (
            <HStack spacing={1} w="full">
              <ModalAssetItem
                ...
                icon={createIconFromDataUri(selectedNetwork.nativeCurrency.iconUrl, {
                  color: subTextColor,
                  h: 3,
                  w: 3,
                })}
              ...
              {/*info*/}
              <InfoIconTooltip color={subTextColor} label={t<string>('captions.optOutFee')} />
            </HStack>
          )}

          <HStack spacing={1} w="full">
            {/*id*/}
            <ModalTextItem flexGrow={1} isCode={true} label={`${t<string>('labels.id')}:`} value={selectedAsset.id} />
  ...
  return (
    <>
      ...
      <Modal isOpen={isOpen} motionPreset="slideInBottom" onClose={onClose} size="full" scrollBehavior="inside">
        <ModalContent
