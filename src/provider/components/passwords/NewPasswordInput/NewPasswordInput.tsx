...
import Link from '@provider/components/generic/Link';
import StrengthMeter from '@provider/components/passwords/StrengthMeter';

// constants
import { DEFAULT_GAP, INPUT_HEIGHT } from '@common/constants';
import { STRONG_PASSWORD_POLICY_LINK } from '@provider/constants';
...
const NewPasswordInput: ForwardRefExoticComponent<PropsWithoutRef<TProps> & RefAttributes<HTMLInputElement>> =
  forwardRef(({ colorMode, error, id, label, score, value, ...inputProps }, ref) => {
        ...
        <VStack px={DEFAULT_GAP - 2} spacing={DEFAULT_GAP / 3} w="full">
          <Text color={subTextColor} fontSize="xs" textAlign="left">
            <Trans i18nKey="captions.passwordScoreInfo">
              To conform with our{' '}
              <Link fontSize="xs" href={STRONG_PASSWORD_POLICY_LINK} isExternal={true}>
                Strong Password Policy
              </Link>
              , you are required to use a sufficiently strong password. Password must be at least 8 characters.
            </Trans>
          </Text>
