import { Box, HStack, Input, Skeleton, Text, type TextProps, VStack } from '@chakra-ui/react';
...
const EditableText: FC<TProps> = ({
  ...
  // memos
  const loadingText = useMemo(() => faker.random.alphaNumeric(12).toUpperCase(), []);
  const charactersRemaining = useMemo(
    () => (characterLimit ? characterLimit - new TextEncoder().encode(value).byteLength : null),
    [value]
  );
  // state
  const [_charactersRemaining, setCharactersRemaining] = useState<number | null>(charactersRemaining);
  ...
  const handleSubmitClick = () => {
    if ((typeof _charactersRemaining === 'number' && _charactersRemaining < 0) || _value.length <= 0) {
      return;
    }
  ...
  return (
    <VStack position="relative" w="full" zIndex={1}>
      ...
      <Input
        borderRadius="full"
        focusBorderColor={
          (typeof _charactersRemaining === 'number' && _charactersRemaining < 0) || _value.length <= 0
            ? 'red.300'
            : primaryColor
        }
      ...
      >
        ...
        {typeof _charactersRemaining === 'number' && (
          <Box bg={textBackgroundColor} borderRadius="full" boxShadow="lg" px={DEFAULT_GAP / 3} py={1}>
            <Text
