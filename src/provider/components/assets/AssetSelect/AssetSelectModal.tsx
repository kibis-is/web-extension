...
// components
import AssetItem from '@provider/components/assets/AssetItem';
...
// selectors
import { useSelectSettingsSelectedNetwork } from '@provider/selectors';
...
const AssetSelectModal: FC<TAssetSelectModalProps> = ({ assets, colorMode, isOpen, multiple, onClose, onSelect }) => {
  ...
  // states
  const [selectedAssets, setSelectedAssets] = useState<(IAssetTypes | INativeCurrency)[]>([]);
  // misc
  ...
  const handleOnChange = (asset: IAssetTypes | INativeCurrency) => () => {
    ...
    if (asset.type === AssetTypeEnum.Native) {
      ...
      if (selectedAssets.find((value) => value.type === AssetTypeEnum.Native)) {
        return setSelectedAssets(selectedAssets.filter((value) => value.type !== AssetTypeEnum.Native));
      }
    ...
    if (selectedAssets.find((value) => value.type !== AssetTypeEnum.Native && value.id === asset.id)) {
      return setSelectedAssets(
        selectedAssets.filter((value) => value.type !== AssetTypeEnum.Native && value.id !== asset.id)
      );
    }
              ...
              <Checkbox
                ...
                isChecked={
                  !!selectedAssets.find((value) => {
                    if (value.type === AssetTypeEnum.Native && asset.type === AssetTypeEnum.Native) {
                      return true;
                    }
        ...
        <ModalHeader display="flex" justifyContent="center" px={DEFAULT_GAP}>
          <VStack spacing={DEFAULT_GAP - 2} w="full">
            ...
            <Heading color={defaultTextColor} size="md" textAlign="center" w="full">
              {t<string>(multiple ? 'headings.selectAssets' : 'headings.selectAsset')}
            </Heading>

            {/*select all assets*/}
            {multiple && (
              <Stack alignItems="flex-end" justifyContent="center" px={DEFAULT_GAP / 2} w="full">
                <Tooltip aria-label={t<string>('labels.selectAllAssets')} label={t<string>('labels.selectAllAssets')}>
                  <Checkbox
                    colorScheme={primaryColorScheme}
                    isChecked={selectedAssets.length === assets.length}
                    isIndeterminate={selectedAssets.length > 0 && selectedAssets.length < assets.length}
