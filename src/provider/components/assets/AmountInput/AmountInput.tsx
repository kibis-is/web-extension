import { Button, HStack, NumberInput, NumberInputField, Text, Tooltip, VStack } from '@chakra-ui/react';
...
import InformationIcon from '@provider/components/icons/InformationIcon';
...
const AmountInput: FC<TProps> = ({
  ...
  const balance = new BigNumber(
    account.networkInformation[convertGenesisHashToHex(network.genesisHash).toUpperCase()]?.atomicBalance || 0
  );
  const _id = id || encodeBase64URLSafe(randomBytes(6));
  const minBalance = new BigNumber(
    account.networkInformation[convertGenesisHashToHex(network.genesisHash).toUpperCase()]?.minAtomicBalance || 0
  );
  ...
  const renderMaximumTransactionAmountLabel = () => {
    ...
    maximumTransactionAmountLabel = (
      <Tooltip aria-label="Full maximum amount" label={maximumTransactionAmountInStandardUnit.toString()}>
        <HStack
        ...
        >
          <Text color={subTextColor} fontSize="xs" textAlign="right">
            {`${t<string>('labels.max')}: ${formatCurrencyUnit(maximumTransactionAmountInStandardUnit, {
    ...
    if (asset.type === AssetTypeEnum.Native) {
      informationLabel = t<string>('captions.maximumNativeCurrencyTransactionAmount', {
        balance: formatCurrencyUnit(convertToStandardUnit(balance, asset.decimals), { decimals: asset.decimals }),
        minBalance: formatCurrencyUnit(convertToStandardUnit(minBalance, asset.decimals), { decimals: asset.decimals }),
        minFee: formatCurrencyUnit(convertToStandardUnit(new BigNumber(network.minFee), asset.decimals), {
          decimals: asset.decimals,
        }),
      ...
      return (
        <HStack alignItems="center" justifyContent="flex-end" spacing={1} w="full">
          <InformationIcon ariaLabel={informationLabel} tooltipLabel={informationLabel} />
    ...
    <VStack alignItems="flex-start" spacing={DEFAULT_GAP / 3} w="full">
      ...
      <HStack spacing={1} w="full">
        ...
        <NumberInput
          ...
          w="full"
        >
          <NumberInputField borderRadius="full" h={INPUT_HEIGHT} p={DEFAULT_GAP - 2} textAlign="right" />
        </NumberInput>
