...
const AccountItem: FC<TProps> = ({ account, colorMode, network, subTextColor, textColor }) => {
  // hooks
  ...
  const renderNameAddress = useCallback(() => {
    ...
    if (enVoiName) {
      return (
        <VStack alignItems="flex-start" flexGrow={1} justifyContent="space-evenly" spacing={0}>
          <Text color={textColor || defaultTextColor} fontSize="sm" maxW={195} noOfLines={1} textAlign="left">
            {enVoiName}
          </Text>
  ...
  }, [account, address, defaultSubTextColor, defaultTextColor, enVoiName, subTextColor, textColor]);
