...
const AddressDisplay: FC<IProps> = ({ accounts, address, ariaLabel, colorScheme, size = 'sm' }) => {
  // selectors
  ...
  const account: IAccount | null =
    accounts.find((value) => convertPublicKeyToAVMAddress(AccountRepository.decode(value.publicKey)) === address) ||
    null;
    ...
    return (
      ...
      >
        ...
        {account.name ? (
          ...
          <TagLabel>
            {ellipseAddress(convertPublicKeyToAVMAddress(AccountRepository.decode(account.publicKey)))}
          </TagLabel>
        )}
      </Tag>
    );
  ...
  return (
    <Tooltip aria-label={ariaLabel} label={address}>
      <Text color={colorScheme ? `${colorScheme || defaultColorScheme}.500` : defaultTextColor} fontSize={size}>
