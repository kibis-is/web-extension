import { Box, Heading, HStack, Input, Skeleton, Stack, Text, Tooltip, useOutsideClick, VStack } from '@chakra-ui/react';
...
const EditableAccountNameField: FC<IProps> = ({
  ...
  // misc
  const reset = () => {
    setCharactersRemaining(ACCOUNT_NAME_BYTE_LIMIT - new TextEncoder().encode(name || '').byteLength);
    setValue(name);
  };
  // handlers
  ...
  const handleOnChange = (event: ChangeEvent<HTMLInputElement>) => {
    const valueAsBytes = new TextEncoder().encode(event.target.value).byteLength;
    ...
    if (name) {
      return (
        <Tooltip aria-label="Name of account" label={name}>
          <Heading color={defaultTextColor} maxW={400} noOfLines={1} size="md" textAlign="left">
            {name}
          </Heading>
        </Tooltip>
      );
  ...
  return (
    <VStack position="relative" ref={containerRef} w="full" zIndex={1}>
      ...
      <Stack bottom="calc(100% + var(--chakra-space-2))" position="absolute" right={DEFAULT_GAP / 3}>
        <Text color={charactersRemaining >= 0 ? subTextColor : 'red.300'} fontSize="xs" textAlign="right" w="full">
      ...
      {/*controls*/}
      <HStack position="absolute" right={0} top="calc(100% + var(--chakra-space-2))">
        {/*submit*/}
        <Box bg={BODY_BACKGROUND_COLOR} borderRadius={theme.radii['md']} boxShadow="lg">
          <IconButton
