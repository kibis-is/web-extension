...
const PasskeysTab: FC<IProps> = ({ account, colorMode, fetching, onRemoveClick, onViewClick }) => {
  ...
  const renderContent = () => {
    ...
    if (account.passkeys && account.passkeys.length > 0) {
      nodes = account.passkeys.map((value) => (
        <Item key={`${_context}-${value.id}`} onRemoveClick={onRemoveClick} onViewClick={onViewClick} passkey={value} />
      ));
    }
