...
import useMinimumBalanceRequirementsForTransactions from '@provider/hooks/useMinimumBalanceRequirementsForTransactions';
...
const KeyRegistrationTransactionModalBody: FC<IProps> = ({
  ...
  const { accountBalanceInAtomicUnits, minimumBalanceRequirementInAtomicUnits, totalCostInAtomicUnits } =
  ...
  // misc
  const feeAsAtomicUnit = new BigNumber(transaction.fee ? String(transaction.fee) : network.minFee);
  ...
  const transactionType: TransactionTypeEnum = parseTransactionType(transaction.get_obj_for_encoding(), {
    network,
    sender: account,
  });
  ...
  return (
    <VStack alignItems="flex-start" justifyContent="flex-start" spacing={DEFAULT_GAP / 3} w="full">
      ...
      <ModalItem
        ...
        value={
          <AddressDisplay accounts={accounts} address={encodeAddress(transaction.from.publicKey)} network={network} />
        }
      />
          ...
          warningLabel: t<string>('captions.minimumBalanceTooLow', {
            ...
            cost: formatCurrencyUnit(
              convertToStandardUnit(
                minimumBalanceRequirementInAtomicUnits.plus(totalCostInAtomicUnits),
      ...
      {condensed && transactionType === TransactionTypeEnum.KeyRegistrationOnline && (
        ...
        >
          <VStack spacing={DEFAULT_GAP / 3} w="full">
            ...
            {transaction.voteFirst && (
              <ModalTextItem label={`${t<string>('labels.voteFirst')}:`} value={String(transaction.voteFirst)} />
            )}

            {/*vote last*/}
            {transaction.voteLast && (
              <ModalTextItem label={`${t<string>('labels.voteLast')}:`} value={String(transaction.voteLast)} />
            )}
          </VStack>
        </MoreInformationAccordion>
      )}
