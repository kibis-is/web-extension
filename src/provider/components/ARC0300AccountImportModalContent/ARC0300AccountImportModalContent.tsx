...
// enums
import { AccountTabEnum, EncryptionMethodEnum } from '@provider/enums';

// errors
import { BaseExtensionError, MalformedDataError } from '@common/errors';

// features
import { saveActiveAccountDetails, saveNewAccountsThunk } from '@provider/features/accounts';
...
// selectors
import { useSelectAccounts, useSelectActiveAccountDetails, useSelectLogger } from '@provider/selectors';
...
import flattenAccountImportSchemaToNewAccounts from '@provider/utils/flattenAccountImportSchemaToNewAccounts';

const ARC0300AccountImportModalContent: FC<IARC0300ModalContentProps<IARC0300AccountImportSchema[]>> = ({
  cancelButtonIcon,
  cancelButtonLabel,
  colorMode,
  onComplete,
  ...
  const handleOnAuthenticationModalConfirm = async (result: TEncryptionCredentials) => {
    ...
    try {
      if (result.type === EncryptionMethodEnum.Unencrypted) {
        throw new MalformedDataError('adding accounts require encryption credentials');
      }
          ...
          description:
            _accounts.length > 1
              ...
              : t<string>('captions.addedAccount', {
                  address: ellipseAddress(convertPublicKeyToAVMAddress(_accounts[0].publicKey)),
                }),
  ...
  const renderBody = () => {
    if (items.length <= 0) {
      return (
        <VStack alignItems="center" flexGrow={1} justifyContent="center" p={DEFAULT_GAP} w="full">
          <Spacer />

          <EmptyState colorMode={colorMode} text={t<string>('captions.noAccountsToImport')} />
      ...
      <VStack spacing={DEFAULT_GAP / 3} w="full">
        {items.map(({ keyPair, name }, index) => (
          <HStack
            ...
            w="full"
          >
            <NewAccountItem address={convertPublicKeyToAVMAddress(keyPair.publicKey())} {...(name && { name })} />

            <Text color={defaultTextColor} fontSize="sm" textAlign="center" w="10%">
              {index + 1}
            </Text>
          </HStack>
        ))}
    ...
    <>
      ...
      <ModalContent backgroundColor={BODY_BACKGROUND_COLOR} borderTopRadius={theme.radii['3xl']} borderBottomRadius={0}>
        ...
        <ModalHeader display="flex" justifyContent="center" px={DEFAULT_GAP}>
          <VStack alignItems="center" flexGrow={1} spacing={DEFAULT_GAP - 2} w="full">
