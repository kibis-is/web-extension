...
const ModalTextItem: FC<IProps> = ({
  ...
  return (
    <HStack alignItems="center" justifyContent="space-between" spacing={DEFAULT_GAP / 3} w="full" {...stackProps}>
      {/*label*/}
      <Text color={defaultTextColor} fontSize={fontSize} w="35%">
        {label}
      </Text>

      <HStack flexGrow={1} justifyContent="flex-end" minH={MODAL_ITEM_HEIGHT} spacing={DEFAULT_GAP / 3} w="full">
        {/*value*/}
        {tooltipLabel ? <Tooltip label={tooltipLabel}>{renderValue()}</Tooltip> : renderValue()}

        {/*copy button*/}
        {copyButtonLabel && (
          <CopyIconButton ariaLabel={copyButtonLabel} tooltipLabel={copyButtonLabel} size="sm" value={value} />
        )}
