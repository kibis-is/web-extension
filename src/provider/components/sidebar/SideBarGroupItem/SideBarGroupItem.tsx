...
import { IoFolderOutline, IoFolderOpenOutline, IoReorderTwoOutline } from 'react-icons/io5';
...
// components
import SideBarAccountItem from '@provider/components/sidebar/SideBarAccountItem';
...
const SideBarGroupItem: FC<IProps> = ({
  ...
  const { isOpen, onToggle } = useDisclosure({
    defaultIsOpen:
      !!activeAccountID &&
      !!accounts.filter(({ groupID }) => groupID === group.id).find(({ id }) => id === activeAccountID),
  });
  ...
  const groupAccounts = useMemo(
    () => AccountRepository.sortByGroupIndex(accounts.filter(({ groupID }) => !!groupID && groupID === group.id)),
    [accounts]
  );
  // states
  const [_accounts, setAccounts] = useState<IAccountWithExtendedProps[]>(groupAccounts); // a local state fixes the delay between the ui and redux updates
  // handlers
  ...
  const handleOnDragEnd = (event: DragEndEvent) => {
    ...
    setAccounts((prevState) => {
      updatedItems = arrayMove(prevState, previousIndex, nextIndex).map((value, index) => ({
        ...value,
        groupIndex: index,
      }));
    ...
    >
            ...
            <HStack m={0} p={0} spacing={DEFAULT_GAP / 3} w="full">
              ...
              <Center minW={`${SIDEBAR_MIN_WIDTH}px`}>
                <Avatar
                  ...
                  size="sm"
                >
                  <AvatarBadge bg={primaryColor} borderWidth={0} boxSize="1.25em" p={1} placement="bottom-end">
                    <Text color={primaryButtonTextColor} fontSize="xxs" textAlign="center">
      ...
      <Collapse in={isOpen} animateOpacity={true} style={{ width: '100%' }}>
        <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleOnDragEnd}>
          <SortableContext items={_accounts} strategy={verticalListSortingStrategy}>
            {_accounts.map((value) => (
              <SideBarAccountItem
                account={value}
                accounts={accounts}
