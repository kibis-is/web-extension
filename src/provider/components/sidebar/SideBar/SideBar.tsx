...
import React, { type FC, type TransitionEvent, useEffect, useState } from 'react';
import {
  IoAddCircleOutline,
  IoChevronBack,
  IoChevronForward,
...
import SideBarSkeletonItem from '@provider/components/sidebar/SideBarSkeletonItem';
...
import { openConfirmModal, setScanQRCodeModal } from '@provider/features/layout';
import { openModal as openManageGroupsModal } from '@provider/features/manage-groups-modal';
...
// types
import type { IAccountGroup, IAccountWithExtendedProps, IAppThunkDispatch, IMainRootState } from '@provider/types';
...
const SideBar: FC = () => {
  ...
  const handleOnAccountSort = (items: IAccountWithExtendedProps[]) => dispatch(saveAccountsThunk(items));
  const handleOnAddToGroupClick = (accountID: string) => dispatch(openMoveGroupModal(accountID));
  const handleOnGroupSort = (items: IAccountGroup[]) => dispatch(saveAccountGroupsThunk(items));
    ...
    dispatch(
      openConfirmModal({
        description: t<string>('captions.removedFromGroupConfirm', {
          account: account.name || ellipseAddress(convertPublicKeyToAVMAddress(account.publicKey)),
          group: group.name,
        }),
  ...
  const handleSendAssetClick = () => {
    ...
    fromAccount = availableAccounts.find((value) => value.id === activeAccount.id) || availableAccounts[0] || null;
    ...
    >
      <HStack justifyContent="flex-end" w="full">
        {isHeaderShowing && (
          <HStack flexGrow={1} px={2} spacing={1} w="full">
            <KibisisIcon boxSize={calculateIconSize('md')} color={primaryColor} />
      ...
      <ScrollableContainer direction="column" flexGrow={1} m={0} p={0} spacing={0} w="full">
        {!network || fetchingAccounts ? (
          Array.from({ length: 3 }, (_, index) => <SideBarSkeletonItem key={`sidebar-fetching-item-${index}`} />)
        ) : (
          <>
            {/*groups*/}
            {groups.length > 0 && (
