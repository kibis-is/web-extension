...
const SideBarAccountList: FC<IProps> = ({
  ...
  // memos
  const accountsWithoutGroup = useMemo(() => sortByIndex(accounts.filter(({ groupID }) => !groupID)), [accounts]);
  // states
  const [_accounts, setAccounts] = useState<IAccountWithExtendedProps[]>(accountsWithoutGroup); // a local state fixes the delay between the ui and redux updates
  // handlers
  ...
  return (
    <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleOnDragEnd}>
      <SortableContext items={accountsWithoutGroup} strategy={verticalListSortingStrategy}>
