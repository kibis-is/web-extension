...
const ReEncryptKeysLoadingContent: FC<IProps> = ({ colorMode, encryptionProgressState, fontSize = 'sm' }) => {
  const { t } = useTranslation();
  // hooks
  const defaultTextColor = useDefaultTextColor();
  const subTextColor = useSubTextColor();
  // misc
  const count = encryptionProgressState.filter(({ encrypted }) => encrypted).length;
  ...
  return (
    <VStack alignItems="center" flexGrow={1} justifyContent="center" spacing={DEFAULT_GAP - 2} w="full">
      ...
      {/*caption*/}
      <Text color={subTextColor} fontSize={fontSize} textAlign="center" w="full">
