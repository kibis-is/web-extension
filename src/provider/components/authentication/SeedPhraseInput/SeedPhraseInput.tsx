...
import React, { type ChangeEvent, type ClipboardEvent, type FC, type ReactNode, useMemo, useState } from 'react';
...
const SeedPhraseInput: FC<IProps> = ({ colorMode, disabled, error, onChange, phrases }) => {
  ...
  return (
    <VStack>
      <HStack alignItems="center" minH={DEFAULT_GAP + 2} spacing={DEFAULT_GAP / 3} w="full">
        {/*label*/}
        <Label
          colorMode={colorMode}
          error={error}
      ...
      <Grid gap={2} templateColumns="repeat(3, 1fr)" w="full">
        {phrases.map((value, index, array) => {
          const input: ReactNode = (
            <InputGroup size="md">
              <InputLeftElement color={subTextColor} h="full" pointerEvents="none" w={DEFAULT_GAP + 2}>
          ...
          if (index >= array.length - 1) {
            return (
              <GridItem colEnd={2} colStart={2} key={`${_context}-seed-phrase-input-item-${index}`}>
                {input}
              </GridItem>
            );
          }

          return <GridItem key={`${_context}-seed-phrase-input-item-${index}`}>{input}</GridItem>;
        })}
      </Grid>
    </VStack>
  );
