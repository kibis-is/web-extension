import { Avatar, AvatarBadge, Button, Center, HStack, Icon, Tag, TagLabel, Text, VStack } from '@chakra-ui/react';
...
const Item: FC<IItemProps> = ({ app, colorMode, network, onClick }) => {
  ...
  const balanceAsStandardUnit = useMemo(
    () => convertToStandardUnit(new BigNumber(app.balance), network.nativeCurrency.decimals),
    [app, network]
  );
  // handlers
  ...
  return (
    <Button
      ...
      pr={1}
      py={0}
      rightIcon={<Icon as={IoChevronForward} boxSize={calculateIconSize('md')} color={defaultTextColor} />}
      variant="ghost"
      w="full"
    >
      <HStack align="center" m={0} p={0} spacing={DEFAULT_GAP / 3} w="full">
        <Avatar
          ...
          icon={<Icon as={IoFlagOutline} color="white" boxSize={calculateIconSize('sm')} />}
          size="sm"
        >
          <AvatarBadge
        ...
        {/*id/address*/}
        <VStack align="flex-start" flexGrow={1} h="100%" justify="space-between" spacing={DEFAULT_GAP / 3}>
        ...
        {/*balance/badges*/}
        <VStack align="flex-end" flexGrow={1} h="100%" justify="space-between" spacing={DEFAULT_GAP / 3}>
          {/*total balance*/}
          <HStack align="center" h="100%" justify="center" spacing={1}>
            <Text color={defaultTextColor} fontSize="sm">
              {formatCurrencyUnit(balanceAsStandardUnit, {
                decimals: balanceAsStandardUnit.gt(1) ? 2 : network.nativeCurrency.decimals,
              })}
            </Text>
          ...
          {app.phase >= 0 && (
            <Tag colorScheme={primaryColorScheme} size="sm" variant={colorMode === 'dark' ? 'solid' : 'subtle'}>
              <TagLabel>{t<string>('labels.stakingPhase', { number: app.phase + 1 })}</TagLabel>
            </Tag>
          )}
        </VStack>
      </HStack>
