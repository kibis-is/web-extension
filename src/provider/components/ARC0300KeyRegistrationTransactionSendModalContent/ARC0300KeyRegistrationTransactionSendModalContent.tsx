...
import { decodeURLSafe as decodeBase64URLSafe, encode as encodeBase64 } from '@stablelib/base64';
...
// enums
import { ARC0300QueryEnum, ErrorCodeEnum, TransactionTypeEnum } from '@provider/enums';

// errors
import { BaseExtensionError, NotEnoughMinimumBalanceError } from '@common/errors';
...
// utils
import createUnsignedKeyRegistrationTransactionFromSchema from '@provider/utils/createUnsignedKeyRegistrationTransactionFromSchema';
...
const ARC0300KeyRegistrationTransactionSendModalContent: FC<
  IARC0300ModalContentProps<
    IARC0300OfflineKeyRegistrationTransactionSendSchema | IARC0300OnlineKeyRegistrationTransactionSendSchema
  >
> = ({ cancelButtonIcon, cancelButtonLabel, colorMode, onComplete, onCancel, schemaOrSchemas: schema }) => {
  ...
  // selectors
  const account = useSelectAccountByAddress(schema.query[ARC0300QueryEnum.Sender]);
  ...
  const [unsignedTransaction, setUnsignedTransaction] = useState<Transaction | null>(null);
  // misc
  const genesisHash = schema.query[ARC0300QueryEnum.GenesisHash];
  const isOnlineKeyRegistrationTransaction = (
    _schema: IARC0300OfflineKeyRegistrationTransactionSendSchema | IARC0300OnlineKeyRegistrationTransactionSendSchema
  ) =>
  ...
  useEffect(() => {
    ...
    if (genesisHash) {
      ...
      _network = networks.find((value) => value.genesisHash === _genesisHash) || null;
          ...
          <VStack alignItems="center" flexGrow={1} spacing={DEFAULT_GAP} w="full">
            <Text color={defaultTextColor} fontSize="s," textAlign="center">
              {t<string>('captions.keyRegistrationURI', {
                status: isOnlineKeyRegistrationTransaction(schema) ? 'online' : 'offline',
              })}
            </Text>
