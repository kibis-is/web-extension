...
import SelectOption from '@provider/components/generic/Select/SelectOption';
...
const Select: FC<IProps> = ({
  ...
  useEffect(() => {
    ...
    index = options.findIndex((_value) => JSON.stringify(_value) === JSON.stringify(value));
  ...
  return (
    <>
      ...
      <VStack align="flex-start" spacing={DEFAULT_GAP / 3} w="full">
        ...
        <Tooltip label={t<string>(buttonTooltipLabel || 'labels.openSelectModal')}>
          <ChakraButton
            _hover={{
              bg: !disabled ? buttonHoverBackgroundColor : disabledBackgroundColor,
            }}
            aria-label={label || 'labels.openSelectModal'}
            alignItems="center"
            backgroundColor={!disabled ? 'transparent' : disabledBackgroundColor}
