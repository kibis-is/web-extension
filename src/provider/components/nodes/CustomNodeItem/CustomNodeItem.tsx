import { HStack, Stack, Tag, TagLabel, TagRightIcon, Text, Tooltip, VStack } from '@chakra-ui/react';
...
const CustomNodeItem: FC<IProps> = ({ item, isActivated, onActivate, onDeactivate, onRemove, onSelect }) => {
  ...
  return (
    ...
    >
      {/*details*/}
      <HStack alignItems="center" flexGrow={1} justifyContent="space-evenly" spacing={DEFAULT_GAP / 3}>
        <VStack alignItems="center" flexGrow={1} justifyContent="space-between" spacing={1} w="full">
          {/*name*/}
          <Tooltip label={item.name}>
            <Text color={defaultTextColor} fontSize="md" maxW={400} noOfLines={1} textAlign="left" w="full">
              {item.name}
            </Text>
          </Tooltip>
          ...
          <HStack spacing={DEFAULT_GAP / 3} w="full">
            ...
            {/*indexer*/}
            <Tag colorScheme={item.indexer ? 'green' : 'orange'} size="sm" variant="solid">
              <TagLabel>indexer</TagLabel>
              <TagRightIcon as={item.indexer ? IoCheckmarkCircleOutline : IoCloseCircleOutline} />
            </Tag>
          </HStack>
        </VStack>
