import { Input, InputGroup, InputRightElement, Tooltip, useDisclosure, VStack } from '@chakra-ui/react';
...
const AddressInput: FC<TProps> = ({
  ...
  const handleOnSelect = (_accounts: IAccountWithExtendedProps[]) => {
    const _value = _accounts[0] ? convertPublicKeyToAVMAddress(_accounts[0].publicKey) : null;
      ...
      <VStack alignItems="flex-start" spacing={DEFAULT_GAP / 3} w="full">
        ...
        <InputGroup size="md">
          ...
          <InputRightElement h={INPUT_HEIGHT}>
            ...
            <Tooltip label={selectButtonLabel || t<string>('labels.selectAccount')}>
              <IconButton
                aria-label={selectButtonLabel || t<string>('labels.selectAccount')}
                borderRadius="full"
                colorMode={colorMode}
                isDisabled={isDisabled}
