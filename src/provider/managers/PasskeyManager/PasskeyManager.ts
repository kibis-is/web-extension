...
// errors
import { PasskeyCreationError, PasskeyNotSupportedError, UnableToFetchPasskeyError } from '@common/errors';
...
export default class PasskeyManager {
  ...
  private static async _generateEncryptionKeyFromInputKeyMaterial({
    ...
    const derivationKey = await crypto.subtle.importKey('raw', inputKeyMaterial, DERIVATION_KEY_ALGORITHM, false, [
      'deriveKey',
    ]);
  ...
  public static async createPasskeyCredential({
    ...
    return {
      algorithm: (credential.response as AuthenticatorAttestationResponse).getPublicKeyAlgorithm(),
      id: PasskeyCredentialRepository.encode(new Uint8Array(credential.rawId)),
      initializationVector: PasskeyCredentialRepository.encode(randomBytes(INITIALIZATION_VECTOR_BYTE_SIZE)),
      name: _name,
      publicKey: publicKey ? PasskeyCredentialRepository.encode(new Uint8Array(publicKey)) : null,
  ...
  public static async decryptBytes({
    ...
    const encryptionKey = await PasskeyManager._generateEncryptionKeyFromInputKeyMaterial({
      inputKeyMaterial,
   ...
   * @static
   */
  public static async encryptBytes({ bytes, inputKeyMaterial, passkey }: IEncryptBytesOptions): Promise<Uint8Array> {
    const encryptionKey = await PasskeyManager._generateEncryptionKeyFromInputKeyMaterial({
      inputKeyMaterial,
      userID: passkey.userID,
    });
