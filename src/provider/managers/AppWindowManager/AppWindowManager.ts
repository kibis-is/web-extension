...
// constants
import { DEFAULT_POPUP_HEIGHT, DEFAULT_POPUP_WIDTH } from '@provider/constants';
...
export default class AppWindowManager {
  ...
  constructor({ appWindowRepository, logger }: INewOptions & IBaseOptions) {
    this._appWindowRepository = appWindowRepository || new AppWindowRepository();
   ...
   * @public
   */
  public async createWindow({ left, searchParams, top, type }: ICreateWindowOptions): Promise<Windows.Window | null> {
    ...
    if (!windowURL) {
      this._logger?.debug(`${AppWindowManager.name}#${_functionName}: unknown app type "${type}"`);
    ...
    if (!_window.id) {
      this._logger?.debug(`${AppWindowManager.name}#${_functionName}: no window id for window "${type}"`);
  ...
  public async hydrate(): Promise<void> {
    ...
    appWindows.forEach((appWindow) => {
      const openWindow: Windows.Window | null = openWindows.find((value) => value.id === appWindow.windowId) || null;
