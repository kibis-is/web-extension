...
// constants
import { SALT_BYTES_SIZE } from '@provider/constants';

// errors
import { DecryptionError, EncryptionError, InvalidPasswordError, MalformedDataError } from '@common/errors';
...
export default class PasswordManager {
  ...
  constructor({ logger, passwordTag, passwordTagRepository }: INewOptions & IBaseOptions) {
    this._logger = logger || null;
    this._passwordTag = passwordTag;
    this._passwordTagRepository = passwordTagRepository || new PasswordTagRepository();
  }
  ...
  public static async decryptBytes({
    ...
    const [nonce, salt, encryptedBytes] = [
      ...
      bytes.slice(secretbox.nonceLength, secretbox.nonceLength + SALT_BYTES_SIZE),
   ...
   * @public
   */
  public async saveNewPassword({ currentPassword, newPassword }: ISaveNewPasswordOptions): Promise<IPasswordTag> {
    ...
    if (!passwordTag) {
      ...
      this._logger?.debug(`${PasswordManager.name}#${_functionName}: ${_error}`);
    ...
    if (!isPasswordValid) {
      this._logger?.debug(`${PasswordManager.name}#${_functionName}: invalid password`);

      throw new InvalidPasswordError();
    }

    this._logger?.debug(`${PasswordManager.name}#${_functionName}: saving new password tag to storage`);
  ...
  public async verifyPassword(password: string): Promise<boolean> {
    ...
    if (!passwordTag) {
      this._logger?.debug(`${PasswordManager.name}#${_functionName}: no password password tag found`);
