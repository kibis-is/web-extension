 ...
 */
export default function useSelectNetworkByGenesisHash(genesisHash: string): INetworkWithTransactionParams | null {
  return useSelector<IMainRootState, INetworkWithTransactionParams | null>(
    (state) => state.networks.items.find((value) => value.genesisHash === genesisHash) || null
  );
}
