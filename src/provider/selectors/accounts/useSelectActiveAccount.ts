...
// types
import type { IAccountWithExtendedProps, IActiveAccountDetails, IMainRootState } from '@provider/types';
...
export default function useSelectActiveAccount(): IAccountWithExtendedProps | null {
  return useSelector<IMainRootState, IAccountWithExtendedProps | null>((state) => {
    ...
    if (activeAccountDetails) {
      account = accounts.find((value) => value.id === activeAccountDetails.accountId) || null;
    }
