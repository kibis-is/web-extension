...
export default function useSelectIsCredentialsRequired(): boolean {
  return useSelector<IBackgroundRootState | IMainRootState, boolean>(({ credentialLock, settings }) => {
    if (settings.security.enableCredentialLock) {
      if (settings.security.credentialLockTimeoutDuration <= 0 || !credentialLock.active) {
        return false;
      }
    }
