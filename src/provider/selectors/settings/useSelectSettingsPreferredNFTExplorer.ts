...
// types
import type { IMainRootState, INetwork } from '@provider/types';
...
export default function useSelectPreferredNFTExplorer(): BaseNFTExplorer | null {
  return useSelector<IMainRootState, BaseNFTExplorer | null>((state) => {
    const selectedNetwork: INetwork | null =
      state.networks.items.find((value) => value.genesisHash === state.settings.general.selectedNetworkGenesisHash) ||
      null;
    ...
    return (
      selectedNetwork.nftExplorers.find(
        (value) =>
          value.id ===
          state.settings.general.preferredNFTExplorerIds[convertGenesisHashToHex(selectedNetwork.genesisHash)]
      ) ||
