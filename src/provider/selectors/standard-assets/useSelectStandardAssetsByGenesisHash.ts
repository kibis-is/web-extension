 ...
 */
export default function useSelectStandardAssetsByGenesisHash(genesisHash: string): IStandardAsset[] {
  return useSelector<IMainRootState, IStandardAsset[]>((state) => {
    if (!state.standardAssets.items) {
      return [];
    }

    return state.standardAssets.items[convertGenesisHashToHex(genesisHash).toUpperCase()] || [];
  });
}
