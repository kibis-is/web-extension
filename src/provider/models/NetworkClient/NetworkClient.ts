import { Algodv2, Indexer, IntDecoding, type SuggestedParams, waitForConfirmation } from 'algosdk';
import BigNumber from 'bignumber.js';

// constants
import { TRANSACTION_CONFIRMATION_ROUNDS } from '@provider/constants';

// contracts
import ARC0072Contract from '@provider/contracts/ARC0072Contract';
import ARC0200Contract from '@provider/contracts/ARC0200Contract';

// enums
import { AssetTypeEnum, ErrorCodeEnum } from '@provider/enums';
...
export default class NetworkClient {
  ...
  public async accountInformationWithDelay({
    ...
    return await this._sendRequestWithDelay({
      delay,
      request: async () =>
        (await algod.accountInformation(address).setIntDecoding(IntDecoding.BIGINT).do()) as IAVMAccountInformation,
    });
  }
  ...
  public async arc0072AssetHoldingsWithDelay({
    ...
    const arc0072Indexer = getRandomItem<BaseARC0072Indexer>(this._network.arc0072Indexers) || null;
   ...
   * @public
   */
  public async block({ delay = 0, nodeID, round }: IBlockWithDelayOptions): Promise<IAVMBlock> {
    ...
    return await this._sendRequestWithDelay({
      ...
      request: async () => {
        ...
        return (await requestBuilder.setIntDecoding(IntDecoding.BIGINT).do()) as IAVMBlock;
      },
    });
  }
  ...
  public async lookupAccountTransactionWithDelay({
    ...
    return await this._sendRequestWithDelay({
      delay,
      request: async () => {
        const requestBuilder = indexer.lookupAccountTransactions(address).limit(limit);
        ...
        return (await requestBuilder.setIntDecoding(IntDecoding.BIGINT).do()) as IAVMAccountTransaction;
      },
    });
  }
  ...
  public async searchApplicationsWithDelay({
    ...
    return await this._sendRequestWithDelay({
      delay,
      request: async () => {
        const requestBuilder = indexer.searchForApplications().index(parseInt(applicationId)).limit(limit);

        if (next) {
          requestBuilder.nextToken(next);
        }

        return (await requestBuilder.setIntDecoding(IntDecoding.BIGINT).do()) as IAVMSearchApplicationsResult;
      },
    });
  }
  ...
  public async searchStandardAssetsWithDelay({
    ...
    return await this._sendRequestWithDelay({
      ...
      request: async () => {
        ...
        return (await requestBuilder.setIntDecoding(IntDecoding.BIGINT).do()) as IAVMSearchAssetsResult;
      },
    });
  }
   ...
   */
  public async sendTransactions({ nodeID, signedTransactions }: ISendTransactionOptions): Promise<string> {
    ...
    this._logger?.debug(`${_functionName}: sending transactions to the network "${this._network.genesisId}"`);

    sentRawTransaction = await algod.sendRawTransaction(signedTransactions).setIntDecoding(IntDecoding.BIGINT).do();
  ...
  public async standardAssetInformationByIDWithDelay({ delay, id, nodeID }: IByIDWithDelayOptions): Promise<IAVMAsset> {
    const algod = this.algodByID(nodeID);

    return await this._sendRequestWithDelay({
      delay,
      request: async () =>
        (await algod.getAssetByID(parseInt(id)).setIntDecoding(IntDecoding.BIGINT).do()) as IAVMAsset,
    });
  }
  ...
  public async status(nodeID: string | null): Promise<IAVMStatus> {
    ...
    return (await algod.status().setIntDecoding(IntDecoding.BIGINT).do()) as IAVMStatus;
  }

  public async suggestedParams(nodeID: string | null): Promise<SuggestedParams> {
    const algod = this.algodByID(nodeID);
   ...
   */
  public async transactionParams(nodeID: string | null): Promise<IAVMTransactionParams> {
    const algodNode = this.algodNodeByID(nodeID);
    let response: Response;

    // use rest api as the
    response = await fetch(
      new URL(`${algodNode.port ? `${algodNode.url}:${algodNode.port}` : algodNode.url}/v2/transactions/params`),
      {
        ...(algodNode.token && {
          headers: {
