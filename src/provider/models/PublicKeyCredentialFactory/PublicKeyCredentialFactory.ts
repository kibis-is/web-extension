...
export default class PublicKeyCredentialFactory {
  ...
  public static generate({
    ...
    let coseAlgorithm = publicKeyCreationOptions.pubKeyCredParams.reduce((acc, value) => {
      // ed21559 is preferred
      if (acc === COSE_ED25519_ALGORITHM) {
        return acc;
      }

      return value.alg === COSE_ED25519_ALGORITHM || value.alg === COSE_ES256_ALGORITHM ? value.alg : acc;
    }, null); // get the cose algorithm type
  ...
  public serializedAttestationCredential(): ISerializedPublicKeyCredential<ISerializedAuthenticatorAttestationResponse> {
    ...
    const clientDataJSON = encodeUTF8(
      JSON.stringify({
        ...
        challenge: encodeBase64URLSafe(decodeBase64(this._challenge)).replace(/=/g, ''), // convert the base64 encoded challenge to url safe with padding removed
        origin: this._passkey.origin,
      })
    );
    const signature = this._keyPair.sign(
