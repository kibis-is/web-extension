...
import { IFetchByAddressOptions, INewOptions, ISCSAccount, ISCSAccountResponse } from './types';

export default abstract class BaseSCSIndexer implements IBaseNetworkServiceProvider {
  // protected variables
  ...
  public async fetchByAddress({ address, logger }: IFetchByAddressOptions): Promise<ISCSAccount[]> {
    ...
    try {
      response = await fetch(`${this._baseURL}${this._apiPath}/accounts?owner=${address}`);
    } catch (error) {
