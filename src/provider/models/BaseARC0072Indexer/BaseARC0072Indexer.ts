...
// errors
import { MalformedDataError, NetworkConnectionError, UnknownError } from '@common/errors';

// types
import type { IARC0072AssetHolding, IBaseNetworkServiceProvider } from '@provider/types';
import type { IFetchTokensByOwnerOptions, INewOptions, ITokensResponse } from './types';

export default abstract class BaseARC0072Indexer implements IBaseNetworkServiceProvider {
  // protected variables
  ...
  public async fetchTokensByOwner({ address, logger }: IFetchTokensByOwnerOptions): Promise<IARC0072AssetHolding[]> {
    ...
    try {
      response = await fetch(`${this._baseURL}${this._apiPath}/tokens?owner=${address}`);
    } catch (error) {
