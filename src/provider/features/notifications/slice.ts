  ...
  reducers: {
    closeById: (state: Draft<IState>, action: PayloadAction<string>) => {
      state.items = state.items.map((value) =>
        value.id === action.payload ? { ...value, showing: false, shown: true } : value
      );
    },
    create: (state: Draft<IState>, action: PayloadAction<IAddNotificationPayload>) => {
      state.items = [
        ...state.items,
        {
    ...
    setShowingById: (state: Draft<IState>, action: PayloadAction<string>) => {
      state.items = state.items.map((value) => (value.id === action.payload ? { ...value, showing: true } : value));
    },
    setShowingConfetti: (state: Draft<IState>, action: PayloadAction<boolean>) => {
...
export const { closeById, create, removeAll, removeById, setShowingById, setShowingConfetti } = slice.actions;
