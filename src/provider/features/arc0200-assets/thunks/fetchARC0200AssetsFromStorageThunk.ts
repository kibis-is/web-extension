...
// types
import type { IARC0200Asset, IBaseAsyncThunkConfig, IMainRootState, IRegistrationRootState } from '@provider/types';
...
> = createAsyncThunk<
  ...
  undefined,
  IBaseAsyncThunkConfig<IMainRootState | IRegistrationRootState>
>(ARC0200AssetsThunkEnum.FetchARC0200AssetsFromStorage, async (_, { getState }) => {
  ...
  await Promise.all(
    networks.map(
      async (network) =>
        (assetItems[convertGenesisHashToHex(network.genesisHash).toUpperCase()] =
          await new ARC0200AssetRepository().fetchByGenesisHash(network.genesisHash))
    )
  );

  logger.debug(`${ARC0200AssetsThunkEnum.FetchARC0200AssetsFromStorage}: fetched arc-0200 assets from storage`);
