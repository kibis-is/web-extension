...
const slice = createSlice({
  extraReducers: (builder) => {
    /**fetch from storage**/
    builder.addCase(fetchFromStorageThunk.fulfilled, (state: IState, action) => {
      state.info = action.payload;
    });
    /**save disable what's on update**/
    builder.addCase(saveDisableWhatsNewOnUpdateThunk.fulfilled, (state: IState, action) => {
      if (state.info) {
        state.info = {
          ...state.info,
          whatsNewInfo: {
    ...
    builder.addCase(savePolisAccountIDThunk.fulfilled, (state: IState, action) => {
      if (state.info) {
        state.info = {
          ...state.info,
    ...
    builder.addCase(saveWhatsNewVersionThunk.fulfilled, (state: IState, action) => {
      if (state.info) {
        state.info = {
          ...state.info,
          whatsNewInfo: {
    ...
    builder.addCase(startPollingForNetworkConnectivityThunk.fulfilled, (state: IState, action) => {
      state.networkConnectivity.pollingID = action.payload;
    });
    /**stop polling for network connectivity**/
    builder.addCase(stopPollingForTransactionsParamsThunk.fulfilled, (state: IState) => {
      state.networkConnectivity.pollingID = null;
    });
    /**update network connectivity**/
    builder.addCase(updateNetworkConnectivityThunk.fulfilled, (state: IState, action) => {
    ...
    builder.addCase(updateNetworkConnectivityThunk.rejected, (state: IState) => {
