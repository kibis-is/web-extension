...
// constants
import { NETWORK_CONNECTIVITY_CHECK_INTERVAL } from '@provider/constants';
...
> = createAsyncThunk<number, undefined, IBaseAsyncThunkConfig<IMainRootState>>(
  ...
  (_, { dispatch, getState }) => {
    ...
    logger.debug(`${ThunkEnum.StartPollingForNetworkConnectivity}: started polling for network connectivity`);

    return window.setInterval(() => dispatch(updateNetworkConnectivityThunk()), NETWORK_CONNECTIVITY_CHECK_INTERVAL);
  }
);
