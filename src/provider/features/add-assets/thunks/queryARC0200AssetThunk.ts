...
// types
import type { IAVMSearchApplicationsResult, IARC0200Asset } from '@provider/types';
import type { IAssetsWithNextToken, IQueryARC0200AssetPayload, IQueryByIdAsyncThunkConfig } from '../types';

// utils
import selectAssetsForNetwork from '@provider/utils/selectAssetsForNetwork';
...
> = createAsyncThunk<IAssetsWithNextToken<IARC0200Asset>, IQueryARC0200AssetPayload, IQueryByIdAsyncThunkConfig>(
  AddAssetThunkEnum.QueryARC200Asset,
  async ({ accountId, applicationId, refresh = false }, { getState, rejectWithValue }) => {
    ...
    if (!network) {
      logger.debug(`${AddAssetThunkEnum.QueryARC200Asset}: no network selected`);

      return rejectWithValue(new NetworkNotSelectedError('attempted to query arc200 assets, but no network selected'));
    }
    ...
    arc200Assets = selectAssetsForNetwork(getState().arc0200Assets.items, network.genesisHash);
    ...
    try {
      ...
      for (let index = 0; index < searchApplicationResult.applications.length; index++) {
        const { id } = searchApplicationResult.applications[index];
        let arc200Asset = arc200Assets.find((value) => value.id === String(id)) || null;
      ...
      // update the result
      return {
        items: refresh ? updatedARC200Assets : [...currentARC200Assets.items, ...updatedARC200Assets],
