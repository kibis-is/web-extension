import { createSlice, Draft, PayloadAction, Reducer, SerializedError } from '@reduxjs/toolkit';
...
// types
import { IARC0200Asset, IAssetTypes, IRejectedActionMeta, IStandardAsset } from '@provider/types';
import { IAssetsWithNextToken, IQueryARC0200AssetPayload, IQueryStandardAssetPayload, IState } from './types';
...
const slice = createSlice({
  extraReducers: (builder) => {
    ...
    builder.addCase(
      ...
      (state: IState, action: PayloadAction<IAssetsWithNextToken<IARC0200Asset>>) => {
    ...
    builder.addCase(
      ...
      (state: IState, action: PayloadAction<IAssetsWithNextToken<IStandardAsset>>) => {
  ...
  reducers: {
    ...
    setAccountId: (state: Draft<IState>, action: PayloadAction<string | null>) => {
      state.accountId = action.payload;
    },
    setConfirming: (state: Draft<IState>, action: PayloadAction<boolean>) => {
      state.confirming = action.payload;
    },
    setSelectedAsset: (state: Draft<IState>, action: PayloadAction<IAssetTypes | null>) => {
...
export const { clearAssets, reset, setAccountId, setConfirming, setSelectedAsset } = slice.actions;
