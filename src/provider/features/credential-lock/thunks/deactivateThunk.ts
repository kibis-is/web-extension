...
>(ThunkEnum.Deactivate, async (credentials, { getState, rejectWithValue }) => {
      ...
      privateKeyItems.map(async (value) => {
        ...
        if (!decryptedPrivateKey) {
          throw new DecryptionError(
            `failed to decrypt private key for account "${convertPublicKeyToAVMAddress(value.publicKey)}"`
          );
        }
  ...
  logger.debug(`${ThunkEnum.Deactivate}: decrypted ${privateKeyItems.length} private keys`);

  await privateKeyRepository.saveMany(privateKeyItems);

  logger.debug(`${ThunkEnum.Deactivate}: saved decrypted private keys to storage`);
