...
// types
import type { IAccountInformation, IBaseAsyncThunkConfig, IMainRootState } from '@provider/types';
...
> = createAsyncThunk<string | null, TUndoReKeyAccountThunkPayload, IBaseAsyncThunkConfig<IMainRootState>>(
  ThunkEnum.UndoReKeyAccount,
  async ({ network, reKeyAccount, ...encryptionOptions }, { getState, rejectWithValue }) => {
    ...
    if (!accountInformation.authAddress) {
      logger.debug(`${ThunkEnum.UndoReKeyAccount}: account "${address}" is not re-keyed`);
