...
> = createAsyncThunk<string, string, IBaseAsyncThunkConfig<IMainRootState>>(
  ...
  async (id, { getState }) => {
    ...
    if (!account) {
      logger.debug(`${ThunkEnum.RemoveAccountById}: account "${id}" does not exist, ignoring`);
    ...
    logger.debug(`${ThunkEnum.RemoveAccountById}: removed account "${id}" from storage`);

    // remove the private key
    await new PrivateKeyRepository().removeByPublicKey(account.publicKey);

    logger.debug(`${ThunkEnum.RemoveAccountById}: removed private key "${account.publicKey}" from storage`);
