...
import { makeAssetTransferTxnWithSuggestedParams, type SuggestedParams, type Transaction } from 'algosdk';
...
import type { TUpdateStandardAssetHoldingsPayload, IUpdateStandardAssetHoldingsResult } from '../types';
...
import updateAccountTransactions from '@provider/utils/updateAccountTransactions';
...
>(
  ...
  async ({ accountId, assets, genesisHash, ...encryptionOptions }, { getState, rejectWithValue }) => {
    ...
    if (!account) {
      logger.debug(`${ThunkEnum.RemoveStandardAssetHoldings}: no account for "${accountId}" found`);

      return rejectWithValue(new MalformedDataError('no account found'));
    }

    if (!online) {
      logger.debug(`${ThunkEnum.RemoveStandardAssetHoldings}: extension offline`);

      return rejectWithValue(
        new OfflineError(
          `attempted to remove standard assets [${assets.map(({ id }) => `"${id}"`).join(',')}], but extension offline`
        )
      );
    }
    ...
    if (!network) {
      logger.debug(`${ThunkEnum.RemoveStandardAssetHoldings}: no network found for "${genesisHash}" found`);
    ...
    filteredAssets = assets.filter((asset) =>
      accountInformation.standardAssetHoldings.some((value) => value.id === asset.id)
    );
    ...
    }).plus(new BigNumber(network.minFee).multipliedBy(filteredAssets.length)); // current minimum account balance + minimum balance requirement of the removed assets + the transaction fees of each remove asset transaction

    // if the account balance is below the minimum required for adding a standard asset, error
    if (accountBalanceInAtomicUnits.lt(minimumBalanceRequirementInAtomicUnits)) {
    ...
    assetHoldingsAboveZeroBalance = accountInformation.standardAssetHoldings.filter(
      (assetHolding) =>
        filteredAssets.some((value) => value.id === assetHolding.id) && new BigNumber(assetHolding.amount).gt(0)
    );
    ...
    account.networkInformation[encodedGenesisHash] = await updateAccountInformation({
      address,
    ...
    account.networkTransactions[encodedGenesisHash] = await updateAccountTransactions({
      address,
      currentAccountTransactions:
        account.networkTransactions[encodedGenesisHash] || AccountRepository.initializeDefaultAccountTransactions(),
      delay: NODE_REQUEST_DELAY, // delay each request by 100ms from the last one, see https://algonode.io/api/#limits
      logger,
      network,
      nodeID,
    ...
    logger.debug(`${ThunkEnum.RemoveStandardAssetHoldings}: saving account "${account.id}" to storage`);
