...
> = createAsyncThunk<IAccountWithExtendedProps | null, string, IBaseAsyncThunkConfig<IMainRootState>>(
  ThunkEnum.RemoveFromGroup,
  async (accountID, { getState }) => {
    ...
    if (!account) {
      logger.debug(`${ThunkEnum.RemoveFromGroup}: no account found for "${accountID}", ignoring`);
    ...
    logger.debug(`${ThunkEnum.RemoveFromGroup}: removed account "${accountID}" from group`);
