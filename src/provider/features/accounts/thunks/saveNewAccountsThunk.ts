...
// utils
import savePrivateKeyItemWithPasskey from '@provider/utils/savePrivateKeyItemWithPasskey';
import savePrivateKeyItemWithPassword from '@provider/utils/savePrivateKeyItemWithPassword';
import isCredentialLockActive from '@provider/utils/isCredentialLockActive';
...
> = createAsyncThunk<
  ...
  IAsyncThunkConfigWithRejectValue<IMainRootState>
>(ThunkEnum.SaveNewAccounts, async ({ accounts, ...encryptionOptions }, { getState, rejectWithValue }) => {
  const logger = getState().system.logger;
  const { credentialLockTimeoutDuration, enableCredentialLock } = getState().settings.security;
  ...
  saveUnencryptedPrivateKey =
    ...
    (enableCredentialLock && credentialLockTimeoutDuration > 0 && !credentialLockActive);
  ...
  for (const { keyPair, name } of accounts) {
    ...
    if (!privateKeyItem) {
      logger.debug(`${ThunkEnum.SaveNewAccounts}: unable to add account "${encodedPublicKey}" (public key), skipping`);
  ...
  logger.debug(`${ThunkEnum.SaveNewAccounts}: saved "${_accounts.length}" new accounts to storage`);
