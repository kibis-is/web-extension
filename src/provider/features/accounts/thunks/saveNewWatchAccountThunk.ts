...
> = createAsyncThunk<
  ...
  IAsyncThunkConfigWithRejectValue<IMainRootState>
>(ThunkEnum.SaveNewWatchAccount, async ({ address, name }, { getState, rejectWithValue }) => {
  ...
  logger.debug(`${ThunkEnum.SaveNewWatchAccount}: validating address "${address}"`);
  ...
  account = accounts.find((value) => value.publicKey === AccountRepository.encode(publicKey)) || null;

  logger.debug(`${ThunkEnum.SaveNewWatchAccount}: checking if "${address}" already exists`);

  // if the account exists, just return it
  if (account) {
    privateKeyItem = await new PrivateKeyRepository().fetchByPublicKey(PrivateKeyRepository.encode(publicKey));
  ...
  logger.debug(`${ThunkEnum.SaveNewWatchAccount}: saving watch account "${address}" to storage`);
  ...
  logger.debug(`${ThunkEnum.SaveNewWatchAccount}: saved watch account "${address}" to storage`);
