...
> = createAsyncThunk<
  ...
  IBaseAsyncThunkConfig<IMainRootState>
>(ThunkEnum.RemoveARC0200AssetHoldings, async ({ accountId, assets, genesisHash }, { getState, rejectWithValue }) => {
  ...
  if (!account) {
    logger.debug(`${ThunkEnum.RemoveARC0200AssetHoldings}: no account for "${accountId}" found`);
  ...
  if (!network) {
    logger.debug(`${ThunkEnum.RemoveARC0200AssetHoldings}: no network found for "${genesisHash}" found`);
  ...
  logger.debug(`${ThunkEnum.RemoveARC0200AssetHoldings}: saving account "${account.id}" to storage`);
