...
import type { IUpdateStandardAssetHoldingsResult, TUpdateStandardAssetHoldingsPayload } from '../types';
...
>(
  ...
  async ({ accountId, assets, genesisHash, ...encryptionOptions }, { getState, rejectWithValue }) => {
    ...
    if (!account) {
      logger.debug(`${ThunkEnum.AddStandardAssetHoldings}: no account for "${accountId}" found`);
    ...
    if (!online) {
      ...
      return rejectWithValue(
        new OfflineError(
          `attempted to add standard assets [${assets.map(({ id }) => `"${id}"`).join(',')}], but extension offline`
        )
      );
    }
    ...
    if (!network) {
      logger.debug(`${ThunkEnum.AddStandardAssetHoldings}: no network found for "${genesisHash}" found`);
    ...
    address = convertPublicKeyToAVMAddress(AccountRepository.decode(account.publicKey));
    accountInformation =
    ...
    accountBalanceInAtomicUnits = new BigNumber(accountInformation?.atomicBalance || '0');
    ...
    }).plus(new BigNumber(network.minFee).multipliedBy(filteredAssets.length)); // current minimum account balance + minimum balance required to add the asset + the transaction fee
    ...
    try {
      ...
      logger.debug(
        `${ThunkEnum.AddStandardAssetHoldings}: sending opt-in transactions to the network for assets [${filteredAssets
          .map(({ id }) => `"${id}"`)
          .join(',')}]`
      );
    ...
    logger.debug(`${ThunkEnum.AddStandardAssetHoldings}: saved account "${account.id}" to storage`);
