...
// types
import type { IAccountGroup, IAccountWithExtendedProps, IBaseAsyncThunkConfig, IMainRootState } from '@provider/types';
import type { IAddToGroupPayload } from '../types';

// utils
import isWatchAccount from '@provider/utils/isWatchAccount/isWatchAccount';
...
> = createAsyncThunk<IAccountWithExtendedProps | null, IAddToGroupPayload, IBaseAsyncThunkConfig<IMainRootState>>(
  ThunkEnum.AddToGroup,
  async ({ accountID, groupID }, { getState }) => {
    ...
    if (!account) {
      logger.debug(`${ThunkEnum.AddToGroup}: no account found for "${accountID}", ignoring`);
    ...
    if (!group) {
      logger.debug(`${ThunkEnum.AddToGroup}: no group found for "${groupID}", ignoring`);
    ...
    groupIndex = accounts.filter(
      (value) => !!value.groupID && value.groupID === group?.id && !!value.groupIndex
    ).length;
    ...
    logger.debug(`${ThunkEnum.AddToGroup}: added account "${accountID}" to group "${groupID}"`);
