...
// repositories
import ActiveAccountRepository from '@provider/repositories/ActiveAccountRepository';

// types
import type { IActiveAccountDetails, IBaseAsyncThunkConfig, IMainRootState } from '@provider/types';

const saveActiveAccountDetails: AsyncThunk<
  IActiveAccountDetails | null, // return
  IActiveAccountDetails, // args
  IBaseAsyncThunkConfig<IMainRootState>
> = createAsyncThunk<IActiveAccountDetails | null, IActiveAccountDetails, IBaseAsyncThunkConfig<IMainRootState>>(
  ThunkEnum.SaveActiveAccountDetails,
  async (activeAccountDetails, { getState }) => {
    const accounts = getState().accounts.items;
    const account = accounts.find((value) => value.id === activeAccountDetails.accountId) || null;
