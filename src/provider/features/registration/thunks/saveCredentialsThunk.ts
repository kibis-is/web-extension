...
> = createAsyncThunk<IAccount[], INewAccount[], IAsyncThunkConfigWithRejectValue<IRegistrationRootState>>(
  ...
  async (accounts, { getState, rejectWithValue }) => {
    ...
    try {
      ...
      logger.debug(`${ThunkEnum.SaveCredentials}: removed previous credentials from storage`);
    ...
    logger.debug(`${ThunkEnum.SaveCredentials}: successfully saved ${privateKeyItems.length} keys to storage`);

    // save the accounts to storage
    await new AccountRepository().saveMany(_accounts);

    logger.debug(`${ThunkEnum.SaveCredentials}: successfully saved ${_accounts.length} accounts to storage`);
