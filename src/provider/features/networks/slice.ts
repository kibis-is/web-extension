...
const slice = createSlice({
  extraReducers: (builder) => {
    ...
    builder.addCase(
      ...
      (state: IState, action: PayloadAction<INetworkWithTransactionParams | null>) => {
        if (action.payload) {
          state.items = state.items.map((value) =>
            value.genesisHash === action.payload?.genesisHash ? action.payload : value
          );
        }
    ...
    builder.addCase(
      ...
      (state: IState, action: PayloadAction<INetworkWithTransactionParams[]>) => {
    ...
    builder.addCase(
      ...
      (state: IState, action: PayloadAction<INetworkWithTransactionParams | null>) => {
        if (action.payload) {
          state.items = state.items.map<INetworkWithTransactionParams>((value) =>
            value.genesisHash === action.payload?.genesisHash ? action.payload : value
          );
        }
    ...
    builder.addCase(updateTransactionParamsForSelectedNetworkThunk.pending, (state: IState) => {
      state.updating = true;
    });
    builder.addCase(updateTransactionParamsForSelectedNetworkThunk.rejected, (state: IState) => {
      state.updating = false;
    });
  },
