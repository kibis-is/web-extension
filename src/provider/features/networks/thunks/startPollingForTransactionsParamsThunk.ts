...
// constants
import { NETWORK_TRANSACTION_PARAMS_REFRESH_INTERVAL } from '@provider/constants';
...
> = createAsyncThunk<number, undefined, { state: IMainRootState }>(
  ...
  (_, { dispatch, getState }) => {
    ...
    logger.debug(`${ThunkEnum.StartPollingForTransactionParams}: started polling for network transaction params`);
