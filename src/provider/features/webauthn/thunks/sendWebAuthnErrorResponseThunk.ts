...
> = createAsyncThunk<
  ...
  IBaseAsyncThunkConfig<IBackgroundRootState | IMainRootState>
>(ThunkEnum.SendWebAuthnErrorResponse, async ({ error, event }, { getState }) => {
  const id = generateUUID();
  const logger = getState().system.logger;
  let message: WebAuthnAuthenticateResponseMessage | WebAuthnRegisterResponseMessage | null = null;
