...
>(
  ...
  async ({ accountID, event, ...encryptionOptions }, { dispatch, getState }) => {
    ...
    if (!account) {
      logger.debug(`${ThunkEnum.SendWebAuthnRegisterResponse}: account "${accountID}" not found`);
    ...
    if (!keyPair) {
      logger.debug(
        `${ThunkEnum.SendWebAuthnRegisterResponse}: private key for "${convertPublicKeyToAVMAddress(
          account.publicKey
        )}" not found`
      );
    ...
    await browser.tabs.sendMessage(
      ...
      new WebAuthnRegisterResponseMessage({
        ...
        result: {
          ...
          credential: publicKeyCredentialFactory.serializedAttestationCredential(),
        },
      })
    );
