...
> = createAsyncThunk<ISession, ISession, IBaseAsyncThunkConfig<IBackgroundRootState | IMainRootState>>(
  ...
  async (session, { getState }) => {
    ...
    logger.debug(`${ThunkEnum.SaveToStorage}: saved session "${session.id}" to storage`);
