// types
import { ISession } from '@provider/types';
 ...
 */
export default function upsertSessions(sessions: ISession[], upsertSessions: ISession[]): ISession[] {
  ...
  const updatedSessions: ISession[] = sessions.map(
    (session) => upsertSessions.find((value) => value.id === session.id) || session
  ); // update the sessions
