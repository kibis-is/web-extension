...
import createUnsignedARC0200TransferTransactions from '@provider/utils/createUnsignedARC0200TransferTransactions';
import createUnsignedPaymentTransactions from '@provider/utils/createUnsignedPaymentTransactions';
import createUnsignedStandardAssetTransferTransactions from '@provider/utils/createUnsignedStandardAssetTransferTransactions';
...
>(
  ...
  async ({ amountInStandardUnits, note, receiverAddress }, { getState, rejectWithValue }) => {
    ...
    if (!online) {
      logger.debug(`${ThunkEnum.CreateUnsignedTransactions}: extension offline`);
    ...
    if (!network) {
      logger.debug(`${ThunkEnum.CreateUnsignedTransactions}: no network selected`);

      return rejectWithValue(new NetworkNotSelectedError('attempted to send transaction, but no network selected'));
    }

    senderAccountInformation = AccountRepository.extractAccountInformationForNetwork(sender, network);
    ...
    try {
      amountInAtomicUnits = convertToAtomicUnit(new BigNumber(amountInStandardUnits), asset.decimals).toFixed(); // convert to atomic units
