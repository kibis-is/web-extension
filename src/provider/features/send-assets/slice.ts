...
// thunks
import { createUnsignedTransactionsThunk, submitTransactionThunk } from './thunks';
...
const slice = createSlice({
  extraReducers: (builder) => {
    ...
    builder.addCase(createUnsignedTransactionsThunk.fulfilled, (state: IState) => {
      state.creating = false;
    });
    builder.addCase(createUnsignedTransactionsThunk.pending, (state: IState) => {
      state.creating = true;
    });
    builder.addCase(createUnsignedTransactionsThunk.rejected, (state: IState) => {
      state.creating = false;
    });
    /** submit transaction **/
  ...
  reducers: {
    initialize: (state: Draft<IState>, action: PayloadAction<IInitializePayload>) => {
    ...
    setAsset: (state: Draft<IState>, action: PayloadAction<IAssetTypes | INativeCurrency | null>) => {
      state.asset = action.payload;
    },
    setSender: (state: Draft<IState>, action: PayloadAction<IAccountWithExtendedProps | null>) => {
