...
// errors
import {
  WebAuthnInvalidPasskeyError,
  WebAuthnInvalidPublicKeyError,
} from '@common/errors';
...
>(
  ...
  async (
    { accountID, event, passkeyID, ...encryptionOptions },
    { dispatch, getState }
  ) => {
    ...
    if (!account) {
      logger.debug(
        `${ThunkEnum.SendWebAuthnAuthenticateResponse}: account "${accountID}" not found`
      );
    ...
    if (!passkey) {
      logger.debug(
        `${
          ThunkEnum.SendWebAuthnAuthenticateResponse
        }: passkey "${passkeyID}" for account "${convertPublicKeyToAVMAddress(
          account.publicKey
        )}" not found`
      );
    ...
    await browser.tabs.sendMessage(
      ...
      new WebAuthnAuthenticateResponseMessage({
        ...
        result: {
          ...
          credential:
            publicKeyCredentialFactory.serializedAssertionCredential(),
        },
      })
    );
