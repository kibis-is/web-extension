...
const StakingAppModal: FC<IProps> = ({ onClose, app, network }) => {
  ...
  const address = useMemo(
    () => (app ? convertPublicKeyToAVMAddress(app.publicKey) : null),
    [app]
  );
  ...
  const handleOnMoreInformationToggle = (value: boolean) =>
    value
      ? onMoreInformationAccordionOpen()
      : onMoreInformationAccordionClose();
  // renders
  const renderContent = () => {
    ...
    balanceInStandardUnits = convertToStandardUnit(
      ...
      <>
        ...
        <VStack align="center" spacing={DEFAULT_GAP - 2} w="full">
          ...
          <Tooltip
            label={t<string>(
              app.status === 'online'
                ? 'captions.stakingAppStatusOnline'
                : 'captions.stakingAppStatusOffline'
            )}
          >
            <Avatar
          ...
          <VStack align="center" spacing={1} w="full">
            ...
            <HStack align="center" spacing={1}>
              ...
              >
                <Heading color={defaultTextColor} size="lg" textAlign="center">
                  {formatCurrencyUnit(balanceInStandardUnits, {
                    decimals: balanceInStandardUnits.lt(1)
                      ? network.nativeCurrency.decimals
                      : 2,
                  })}
                </Heading>
              ...
              <InfoIconTooltip
                color={subTextColor}
                label={t<string>('captions.stakingAppBalanceInfo')}
              />
            </HStack>
            ...
            <HStack align="center" spacing={1}>
              ...
              >
                <Text color={subTextColor} textAlign="center">
                  {formatCurrencyUnit(availableBalanceInStandardUnits, {
                    decimals: availableBalanceInStandardUnits.lt(1)
                      ? network.nativeCurrency.decimals
                      : 2,
                  })}
                </Text>
              ...
              <InfoIconTooltip
                label={t<string>('captions.stakingAppAvailableBalanceInfo')}
                color={subTextColor}
              />
            </HStack>
          </VStack>
        ...
        <VStack align="center" spacing={0} w="full">
          <ModalSubHeading
            color={defaultTextColor}
            text={t<string>('headings.details')}
          />
          ...
          >
            <VStack align="center" spacing={0} w="full">
              {/*lock-up years*/}
              <ModalTextItem
                label={t<string>('labels.lockUpYears')}
                value={app.lockupYears.toString()}
              />

              {/*lock-up expires at*/}
              <ModalTextItem
                label={t<string>('labels.lockUpExpiresAt')}
                value={lockUpExpiresAt.toLocaleString()}
              />
              ...
              {app.participationKeyExpiresAt && (
                <ModalTextItem
                  ...
                  value={new Date(
                    parseInt(app.participationKeyExpiresAt, 10)
                  ).toLocaleString()}
                  warningLabel={t<string>(
                    'captions.participationKeyExpiresAtWarning'
                  )}
                />
              )}
            </VStack>
